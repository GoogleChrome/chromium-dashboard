...
from internals import core_enums, core_models, search_fulltext
...
class SearchFulltextFunctionsTest(testing_config.CustomTestCase):
    ...
    def test_get_strings__specific_field(self):
        ...
        self.assertEqual(['creator@example.com'], actual)

        actual = search_fulltext.get_strings(self.fe, 'owner')
        self.assertEqual(['owner1@example.com', 'owner2@example.com'], actual)
    ...
    def test_parse_words__empty(self):
        ...
        self.assertEqual((set(), 0), search_fulltext.parse_words([]))
        self.assertEqual((set(), 0), search_fulltext.parse_words(['']))
        self.assertEqual((set(), 0), search_fulltext.parse_words(['', '']))
    ...
    def test_canonicalize_string(self):
        ...
        self.assertEqual(search_fulltext.canonicalize_string(''), '  ')
        self.assertEqual(
            search_fulltext.canonicalize_string('abc xyz'), ' abc xyz '
        )
        self.assertEqual(
            search_fulltext.canonicalize_string(' Abc    xYz'), ' abc xyz '
        )
