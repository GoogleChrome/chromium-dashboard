...
def notify_subscribers_and_save_amendments(
    ...
    if notify:
        notify_feature_subscribers_of_changes(
            fe, amendments, is_update=is_update
        )
...
def notify_approvers_of_reviews(
    fe: 'FeatureEntry', gate: Gate, new_state: int, email: str
) -> None:
    """Notify approvers of a review requested from a Gate."""
    new_value_str = Vote.VOTE_VALUES.get(new_state, 'None')
    amendment = Amendment(
        field_name='review_status', old_value='None', new_value=new_value_str
    )
    gate_id = gate.key.integer_id()
    activity = Activity(
        feature_id=fe.key.integer_id(),
...
def notify_subscribers_of_vote_changes(
    ...
    stage_name = core_enums.INTENT_STAGES[stage_enum]  # noqa: F841
    ...
    gate_name = appr_def.name  # noqa: F841
...
def notify_subscribers_of_new_comments(
    fe: 'FeatureEntry', gate: Gate, email: str, content: str
) -> None:
...
def notify_approvals(feature: 'FeatureEntry', stage: Stage, gate: Gate) -> None:
    """Send notifications when important approvals have been obtained"""  # noqa: D415
    ...
    if gate.gate_type == core_enums.GATE_API_ORIGIN_TRIAL:
        ...
        if all(g.state in Gate.APPROVED_STATES for g in all_ot_gates):
            send_trial_creation_approved_notification(feature, stage)
...
def send_trial_extension_approved_notification(
    ...
    if ot_stage is None:
        logging.error(
            'No origin trial stage found for given OT stage ID',
            stage.ot_stage_id,
        )
        return
    ...
    cloud_tasks_helpers.enqueue_task(
        '/tasks/email-ot-extension-approved', params
    )


def send_trial_creation_approved_notification(
    fe: 'FeatureEntry', stage: Stage
) -> None:
    """Notify that a trial extension is ready to be finalized."""
    params = {
        'feature': converters.feature_entry_to_json_verbose(fe),
    ...
    cloud_tasks_helpers.enqueue_task(
        '/tasks/email-ot-creation-approved', params
    )
