        ...
        if new_val != old_val:
            ...
            if urls_to_remove or urls_to_add:
                ...
                for url in urls_to_add:
                    link = Link(url)
                    if link.type:
                        feature_link = _get_index_link(
                            link, fe, should_parse_new_link=True
                        )
                        if feature_link:
                            feature_link.put()
                            logging.info(
    ...
    if feature_link.type == LINK_TYPE_GITHUB_ISSUE:
        if possible_entries is None:
            possible_entries = ndb.get_multi(
                keys=[
                    ndb.Key('FeatureEntry', id)
                    for id in feature_link.feature_ids
                ]
            )
        for fe in possible_entries:
            ...
            if (
                ...
                _put_if_changed(
                    ...
                    _get_review_result_from_feature_link(
                        feature_link, 'resolution: '
                    ),
                )
            if (
    ...
    for feature_link in feature_links:
        for feature_id in feature_link['feature_ids']:
            flattened_feature_links.append(
                {**feature_link, 'feature_ids': feature_id}
            )
