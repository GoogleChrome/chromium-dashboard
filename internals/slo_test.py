...
class SLORecordingTests(testing_config.CustomTestCase):
    ...
    def test_record_vote__feature_owner_starting(self):
        ...
        self.assertEqual(
            self.vote_review_requested.set_on, self.gate.requested_on
        )
        self.assertIsNone(self.gate.responded_on)
        self.assertIsNone(self.gate.resolved_on)
        self.assertIsNone(self.gate.needs_work_started_on)
    ...
    def test_record_vote__got_response(self):
        ...
        self.assertTrue(
            slo.record_vote(
                self.gate,
                [self.vote_review_requested, self.vote_needs_work],
    ...
    def test_record_vote__resolving(self):
        ...
        self.assertTrue(
            slo.record_vote(
                self.gate,
                [self.vote_review_requested, self.vote_approved],
                Vote.REVIEW_REQUESTED,
            )
        )
        self.assertEqual(
    ...
    def test_record_vote__reviewer_immediate_resolution(self):
        ...
        self.gate.state = Vote.APPROVED
        self.assertTrue(
            slo.record_vote(self.gate, [self.vote_approved], Gate.PREPARING)
        )
    ...
    def test_record_vote__already_responded(self):
        ...
        self.assertFalse(
            slo.record_vote(
                self.gate,
                [self.vote_review_requested, self.vote_started],
                Vote.REVIEW_REQUESTED,
            )
        )
    ...
    def test_record_comment__not_started(self, mock_crg):
        ...
        self.assertFalse(
            slo.record_comment(feature, self.gate, user, approvers)
        )
...
class SLOReportingTests(testing_config.CustomTestCase):
    def setUp(self):
        ...
        self.gate_2.requested_on = datetime.datetime(
            2023, 6, 7, 12, 30, 0
        )  # Wed
        self.gate_2.put()

        self.gate_3 = Gate(feature_id=2, stage_id=2, gate_type=54, state=4)
        self.gate_3.requested_on = datetime.datetime(
            2023, 6, 9, 12, 30, 0
        )  # Fri
        self.gate_3.put()
