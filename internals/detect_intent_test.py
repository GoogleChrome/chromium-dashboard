...
class FunctionTest(testing_config.CustomTestCase):
    ...
    def test_detect_feature_id__generated(self):
        ...
        self.assertEqual(
            5144822362931200, detect_intent.detect_feature_id(body)
        )
    ...
    def test_detect_lgtm__bad(self):
        ...
        self.assertFalse(
            detect_intent.detect_lgtm('> LGTM from other approver')
        )
...
class IntentEmailHandlerTest(testing_config.CustomTestCase):
    def setUp(self):
        ...
        feature_stages_and_gates = core_enums.STAGES_AND_GATES_BY_FEATURE_TYPE[
            0
        ]
        ...
        for s_type, gate_types in feature_stages_and_gates:
            ...
            for gate_type in gate_types:
                ...
                gate.put()
        extra_extension_stage = Stage(
            feature_id=self.feature_id, stage_type=151
        )
        extra_extension_stage.put()
        extra_extension_gate = Gate(
            feature_id=self.feature_id,
        ...
        self.stages_dict[151][
            0
        ].intent_thread_url = 'https://example.com/exists'
        ...
        self.footer = (
            ...
            'instructions...\n'
            '---\n'
            'To view this discussion visit ' + self.thread_url + '.'
        )
    ...
    def test_process_post_data__gate_in_link(self, mock_info):
        ...
        self.assertEqual('user@example.com', vote.set_by)
        self.assertEqual(
            self.stages_dict[151][0].intent_thread_url, self.thread_url
        )

    @mock.patch('logging.info')
    def test_process_post_data__incorrect_detected_gate(self, mock_info):
        """If a bad gate ID is detected, no votes are generated."""  # noqa: D415
        extend_json_data = {
            'from_addr': 'user@example.com',
            'subject': 'Intent to Extend Experiment: Featurename',
            'body': (
                'Please review. ' + self.entry_link_with_bad_gate + self.footer
            ),
        }
        with test_app.test_request_context(
            self.request_path, json=extend_json_data
        ...
        self.assertEqual(
            ...
            {
                'message': (
                    'Stage not found for intent type 3 '
    ...
    def test_record_slo__gate_not_found(self, mock_info):
        ...
        ship_gate: Gate = Gate.query(
            Gate.feature_id == self.feature_id,
            Gate.gate_type == appr_field.gate_type,
        ).get()
    ...
    def test_record_slo__gate_ambiguous(self, mock_info):
        ...
        gate_3 = Gate(
            feature_id=self.feature_id,
            stage_id=2,
            gate_type=appr_field.gate_type,
            state=Vote.NA,
        )
        gate_3.put()
        gate_4 = Gate(
