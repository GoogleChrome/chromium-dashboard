...
class BackfillStageCreated(FlaskHandler):
    def get_template_data(self, **kwargs) -> str:
        ...
        for stage in stages:
            ...
            if feature_entry == None or stage.created != None:  # noqa: E711
                continue
...
class AssociateOTs(FlaskHandler):
    ...
    def write_fields_for_trial_stage(
        ...
        if trial_stage.ot_setup_status != OT_ACTIVATED and trial_activated:  # noqa: F405
            trial_stage.ot_setup_status = OT_ACTIVATED  # noqa: F405
            stage_changed = True
    ...
    def find_unassociated_trial_stage(self, feature_id: int) -> Stage | None:
        ...
        trial_stage_type = STAGE_TYPES_ORIGIN_TRIAL[fe.feature_type]  # noqa: F405
    ...
    def clear_extension_requests(
        ...
        """Clear any trial extension requests if they have been processed"""  # noqa: D415
    ...
    def get_template_data(self, **kwargs) -> str:
        """Link existing origin trials with their ChromeStatus entry"""  # noqa: D415
        self.require_cron_header()
...
class BackfillFeatureEnterpriseImpact(FlaskHandler):
    def get_template_data(self, **kwargs) -> str:
        ...
        # Update enterprise_impact to be the highest impact set on any of the rollout steps.  # noqa: E501
        for stage in stages:
            feature_entry = features_by_id[stage.feature_id]
            if feature_entry == None:  # noqa: E711
                continue
        ...
        # Set all enterprise features and former breaking changes to have a low impact if no rollout step was step.  # noqa: E501
        ...
        for feature_entry in features:
            ...
            feature_entry.enterprise_impact = ENTERPRISE_IMPACT_MEDIUM  # noqa: F405
...
class CreateOriginTrials(FlaskHandler):
    ...
    def handle_creation(self, stage: Stage) -> bool:
        ...
        if error_text:
            ...
            stage.ot_setup_status = OT_CREATION_FAILED  # noqa: F405
    ...
    def handle_activation(self, stage: Stage) -> None:
        ...
        try:
            ...
            stage.ot_setup_status = OT_ACTIVATED  # noqa: F405
            self._send_creation_result_notification(
            ...
            stage.ot_setup_status = OT_ACTIVATION_FAILED  # noqa: F405
            self._send_creation_result_notification(
...
class BackfillActivityLogType(FlaskHandler):
    def get_template_data(self, **kwargs) -> str:
        ...
        for activity in Activity.query(Activity.log_type == None):  # noqa: E711
            # 1. If the content field is null, the log_type field should be USER_CHANGE.  # noqa: E501
            if not activity.content:
                activity.log_type = Activity.USER_CHANGE
            # 2. If the content field is not null and the string starts with "Shipping/Rollout milestones were unset", the log_type field should be MILESTONE_RESET.  # noqa: E501
            elif activity.content.startswith(
                'Shipping/Rollout milestones were unset'
            ):  # noqa: E501
                activity.log_type = Activity.MILESTONE_RESET
            # 3. If the content field is not null and the amendments field is not empty, the log_type field should be SYSTEM_CHANGE.  # noqa: E501
            elif activity.content and activity.amendments:
                activity.log_type = Activity.SYSTEM_CHANGE
            # 4. If the content field is not null and the amendments field is empty, the log_type field should be USER_COMMENT.  # noqa: E501
...
class SendManualOTCreatedEmail(FlaskHandler):
    ...
    def get_template_data(self, **kwargs):
        ...
        if stage.stage_type not in ALL_ORIGIN_TRIAL_STAGE_TYPES:  # noqa: F405
...
class GenerateReviewActivityFile(FlaskHandler):
    ...
    VOTE_VALUE_MAPPING: dict[str, SkyhookDashStatus] = {  # noqa: F405
        'na': SkyhookDashStatus.FYI,  # noqa: F405
        'review_requested': SkyhookDashStatus.PENDING_REVIEW,  # noqa: F405
        'review_started': SkyhookDashStatus.PENDING_REVIEW,  # noqa: F405
        'needs_work': SkyhookDashStatus.NEEDS_WORK,  # noqa: F405
        'approved': SkyhookDashStatus.APPROVED,  # noqa: F405
        'denied': SkyhookDashStatus.DENIED,  # noqa: F405
        'internal_review': SkyhookDashStatus.PENDING_REVIEW,  # noqa: F405
        'na (self-certified)': SkyhookDashStatus.FYI,  # noqa: F405
        'na_requested': SkyhookDashStatus.PENDING_REVIEW,  # noqa: F405
        'na (verified)': SkyhookDashStatus.FYI,  # noqa: F405
        'no_response': SkyhookDashStatus.PENDING_REVIEW,  # noqa: F405
    }
...
class GenerateStaleFeaturesFile(FlaskHandler):
    """Generate a CSV file with all stale features that have upcoming shipping milestones."""  # noqa: E501
    ...
    def _gather_stale_features(
        ...
        for f in stale_features:
            ...
            # (This cannot be added to the query, since only 1 inequality is allowed per query.)  # noqa: E501
            if f.outstanding_notifications == 0:
                continue
            shipping_stage_type = STAGE_TYPES_SHIPPING[f.feature_type]  # noqa: F405
    ...
    def get_template_data(self, **kwargs) -> str:
        ...
        return f'{len(feature_csv_rows)} rows added to chromestatus-stale-features.csv'  # noqa: E501


class GenerateShippingFeaturesFile(FlaskHandler):
    """Generate a CSV file with all shipping features and information about their missing fields."""  # noqa: E501
    ...
    def _write_csv(
        ...
        """Write shipping features CSV, owners CSV, and missing criteria CSV to the GCP bucket."""  # noqa: E501
        self._upload_csv(
            bucket,
...
class ResetOutstandingNotifications(FlaskHandler):
    ...
    def get_template_data(self, **kwargs) -> str:
        ...
        for f in notified_features:
            logging.info(
                f'Setting outstanding notifications for feature {f.key.integer_id()} '  # noqa: E501
...
class ResetStaleShippingMilestones(FlaskHandler):
    """Reset the shipping milestones of features have not been verified after 4+ notifications."""  # noqa: E501
    ...
    def get_template_data(self, **kwargs) -> str:
        ...
        for f in stale_features:
            ...
            stages: list[Stage] = Stage.query(
                Stage.feature_id == f.key.integer_id(),
                ndb.OR(
                    Stage.stage_type == STAGE_TYPES_SHIPPING[f.feature_type],  # noqa: F405
                    Stage.stage_type == STAGE_ENT_ROLLOUT,  # noqa: F405
                ),
            ).fetch()
            for s in stages:
                # Create an activity that shows all the shipping milestones have been set to null.  # noqa: E501
                activity = Activity(
                ...
                if s.milestones:
                    ...
                    # Only update the stage and save the activity if some milestones were unset.  # noqa: E501
...
class DeleteWPTCoverageReport(FlaskHandler):
    ...
    def get_template_data(self, **kwargs) -> str:
        ...
        # Query for features with a WPT evaluation timestamp older than the threshold.  # noqa: E501
        ...
        for feature in query:
            ...
            if feature.ai_test_eval_report:
                activity = Activity(
                    ...
                    content=(
                        f'WPT coverage report was deleted due to {self.RETENTION_DAYS}-day '  # noqa: E501
                        'retention policy.'
                    ),
                    amendments=[],
                )
                ...
                feature.ai_test_eval_run_status = AITestEvaluationStatus.DELETED  # noqa: F405
