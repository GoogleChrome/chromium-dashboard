...
def format_email_body(
    ...
    additional_template_data: dict[str, Any] | None = None,
) -> str:
...
def accumulate_reasons(
    addr_reasons: dict[str, list], addr_list: list[str], reason: str
) -> None:
...
def convert_reasons_to_task(
    ...
    footer_lines.append(
        '<p><a href="%ssettings">Unsubscribe</a></p>' % settings.SITE_URL
    )
...
def add_core_receivers(fe: FeatureEntry, addr_reasons: dict[str, list[str]]):
    ...
    accumulate_reasons(
        addr_reasons, fe.cc_emails, "You are CC'd on this feature"
    )
    accumulate_reasons(
        addr_reasons,
        fe.devrel_emails,
...
def make_feature_changes_email(
    ...
    accumulate_reasons(
        addr_reasons, watcher_emails, 'You are watching all feature changes'
    )
    ...
    for component_name in fe.blink_components:
        ...
        subscriber_emails: list[str] = [
            sub.email for sub in component.subscribers
        ]
        accumulate_reasons(
            addr_reasons,
            owner_emails,
...
class FeatureStar(ndb.Model):
    ...
    def set_star(self, email, feature_id, starred=True):
        ...
        if feature_entry.star_count < 0:
            logging.error(
                'count would be < 0: %r', (email, feature_id, starred)
            )
            return
        feature_entry.put()
...
class NotifyInactiveUsersHandler(basehandlers.FlaskHandler):
    """Handler for NotifyInactiveUsers requests."""
    ...
    def get_template_data(self, **kwargs):
        ...
        message_parts = [
            f'{len(email_tasks)} users notified of inactivity.',
            'Notified users:',
        ]
...
class FeatureChangeHandler(basehandlers.FlaskHandler):
    ...
    def process_post_data(self, **kwargs):
        ...
        logging.info(
            'Starting to notify subscribers for feature %s',
...
class FeatureReviewHandler(basehandlers.FlaskHandler):
    ...
    def process_post_data(self, **kwargs):
        ...
        logging.info(
            'Starting to notify reviewers for feature %s',
            repr(feature)[: settings.MAX_LOG_LINE],
        )
    ...
    def make_review_requests_email(
        ...
        all_tasks = [
            convert_reasons_to_task(addr, reasons, email_html, subject, None)
            for addr, reasons in sorted(addr_reasons.items())
        ]
        return all_tasks
...
class ReviewAssignmentHandler(basehandlers.FlaskHandler):
    ...
    def process_post_data(self, **kwargs):
        ...
        logging.info(
            'Starting to notify assignees for feature %s',
            repr(feature)[: settings.MAX_LOG_LINE],
        )
...
class FeatureCommentHandler(basehandlers.FlaskHandler):
    ...
    def process_post_data(self, **kwargs):
        ...
        logging.info(
            'Starting to notify of comments for feature %s',
            repr(feature)[: settings.MAX_LOG_LINE],
        )
...
class OTCreationProcessedHandler(basehandlers.FlaskHandler):
    ...
    def build_email(self, stage: dict[str, Any], contacts: list[str]) -> dict:
        ...
        return {
            ...
            'cc': [core_enums.OT_SUPPORT_EMAIL],
            'subject': (
                f'{stage["ot_display_name"]} origin trial has been created '
...
class OTCreationRequestFailedHandler(basehandlers.FlaskHandler):
    ...
    def build_email(self, stage: StageDict, error_text: str | None) -> dict:
        ...
        return {
            ...
            'subject': (
                'Automated trial creation request failed for '
...
class OTActivationFailedHandler(basehandlers.FlaskHandler):
    ...
    def build_email(self, stage: StageDict) -> dict:
        ...
        return {
            'to': core_enums.OT_SUPPORT_EMAIL,
            'subject': (
                'Automated trial activation request failed for '
                f'{stage["ot_display_name"]}'
            ),
            'reply_to': None,
            'html': body,
...
class OTExtensionApprovedHandler(basehandlers.FlaskHandler):
    ...
    def process_post_data(self, **kwargs):
        ...
        logging.info(
            'Starting to notify about successful origin trial extension.'
        )
        send_emails(
            [
                self.build_email(
...
class OTExtendedHandler(basehandlers.FlaskHandler):
    ...
    def build_email(self, extension_stage, ot_stage):
        ...
        return {
            'to': core_enums.OT_SUPPORT_EMAIL,
            'subject': (
                'Origin trial extension processed: '
...
TEST_ARCHIVE_URL_PREFIX = 'https://groups.google.com/d/msgid/jrobbins-test/'
...
def find_bounced_emails(email_tasks):
    ...
    user_prefs = UserPref.get_prefs_for_emails(emails)
    return {up.email for up in user_prefs if up.bounced}
...
def post_comment_to_mailing_list(
    ...
    stage: Stage | None = (
        None if gate is None else Stage.get_by_id(gate.stage_id)
    )
...
class ResetShippingMilestonesEmailHandler(basehandlers.FlaskHandler):
    ...
    def process_post_data(self, **kwargs):
        ...
        logging.info(
            'Starting to notify about shipping/rollout milestone reset.'
        )
