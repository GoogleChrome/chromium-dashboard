...
def build_trial_data(
    ...
    if len(contact_list) == 0:
        logging.warning(
            f'No contacts found for origin trial ID {trial_data["id"]}'
        )
        return None
...
def send_email_reminders() -> str:
    ...
    cloud_tasks_helpers.enqueue_task(
        '/tasks/email-ot-automated-process', params
    )
...
def send_branch_emails(release: int, next_branch_date: date) -> int:
    ...
    for trial in starting_trials:
        ...
        cloud_tasks_helpers.enqueue_task('/tasks/email-ot-first-branch', params)

    num_last_release_trials = len(ending_trials)
    logging.info(
        f'Entering last release - {num_last_release_trials} emails to send'
    )
    for trial in ending_trials:
        ...
        cloud_tasks_helpers.enqueue_task('/tasks/email-ot-last-branch', params)
...
def get_branch_date(release_json: dict[str, str]) -> date:
    ...
    return datetime.strptime(
        release_json['branch_point'].split('T')[0], '%Y-%m-%d'
    ).date()
...
def get_beta_date(release_json: dict[str, str]) -> date:
    ...
    return datetime.strptime(
        release_json['earliest_beta'].split('T')[0], '%Y-%m-%d'
    ).date()
...
def get_stable_date(release_json: dict[str, str]) -> date:
    ...
    return datetime.strptime(
        release_json['stable_date'].split('T')[0], '%Y-%m-%d'
    ).date()
