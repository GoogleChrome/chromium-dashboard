...
class OTProcessRemindersTest(testing_config.CustomTestCase):
    ...
    def setUp(self):
        ...
        self.stage_1 = Stage(
            feature_id=1,
            stage_type=150,
            ot_owner_email='ot_owner1@google.com',
        ...
        self.stage_2 = Stage(
            feature_id=2,
            stage_type=150,
            ot_owner_email='ot_owner2@google.com',
    ...
    def test_build_trials__normal(self):
        ...
        self.assertEqual(
            expected_trials[0]['start_milestone'],
            formatted_1['start_milestone'],
        )
        self.assertEqual(
            expected_trials[1]['start_milestone'],
            formatted_2['start_milestone'],
        )
        self.assertEqual(
            set(expected_trials[0]['contacts']), set(formatted_1['contacts'])
        )
        self.assertEqual(
            set(expected_trials[1]['contacts']), set(formatted_2['contacts'])
        )
    ...
    def test_get_trials(self, mock_get_trials_list):
        ...
        self.assertEqual(
            starting_trial['id'], expected_starting_trials[0]['id']
        )
        self.assertEqual(ending_trial['id'], expected_ending_trials[0]['id'])
        self.assertEqual(
            starting_trial['name'], expected_starting_trials[0]['name']
        )
        self.assertEqual(
            ending_trial['name'], expected_ending_trials[0]['name']
        )
        self.assertEqual(
            set(starting_trial['contacts']),
            set(expected_starting_trials[0]['contacts']),
        )
        self.assertEqual(
            set(ending_trial['contacts']),
