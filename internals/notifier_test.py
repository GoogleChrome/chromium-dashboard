...
class EmailFormattingTest(testing_config.CustomTestCase):
    ...
    def setUp(self):
        ...
        self.fe_2_ship_stage = Stage(
            feature_id=self.fe_2.key.integer_id(),
            stage_type=260,
            milestones=MilestoneSet(),
        )
        self.fe_2_ship_stage.put()
        self.fe_2_stages = stage_helpers.get_feature_stages(
            self.fe_2.key.integer_id()
        )  # noqa: E501
        ...
        self.template_fe = FeatureEntry(
            ...
            editor_emails=['feature_editor@example.com', 'owner_1@example.com'],
            category=1,
            creator_email='creator_template@example.com',
            updater_email='editor_template@example.com',
            blink_components=['Blink'],
            feature_type=0,
        )
        self.template_ship_stage = Stage(
            feature_id=123,
            stage_type=160,
            milestones=MilestoneSet(desktop_first=100),
        )
        self.template_ship_stage_2 = Stage(
            feature_id=123,
            stage_type=160,
    ...
    def test_format_email_body__update_no_changes(self):
        ...
        # TESTDATA.make_golden(body_html, 'test_format_email_body__update_no_changes.html')  # noqa: E501
        self.assertEqual(
            body_html,
    ...
    def test_format_email_body__update_with_changes(self):
        ...
        # TESTDATA.make_golden(body_html, 'test_format_email_body__update_with_changes.html')  # noqa: E501
        self.assertEqual(
            body_html,
    ...
    def test_format_email_body__update_with_changes_and_note(self):
        ...
        # TESTDATA.make_golden(body_html, 'test_format_email_body__update_with_changes_and_note.html')  # noqa: E501
        self.assertEqual(
            body_html,
            TESTDATA[
    ...
    def test_accumulate_reasons(self):
        ...
        self.assertEqual({'owner_1@example.com': ['a reason']}, addr_reasons)

        notifier.accumulate_reasons(
            addr_reasons,
            [self.component_owner_1.email, self.watcher_1.email],
            'another reason',
        )
        self.assertEqual(
            {
    ...
    def test_convert_reasons_to_task__normal(self):
        ...
        self.assertCountEqual(
            ['to', 'subject', 'html', 'reply_to'], list(actual.keys())
        )
    ...
    def test_apply_subscription_rules__iwa_match(self):
        ...
        self.assertEqual(
            {notifier.IWA_RULE_REASON: notifier.IWA_RULE_ADDRS}, actual
        )
    ...
    def test_apply_subscription_rules__relevant_match(self):
        ...
        self.assertEqual(
            {notifier.WEBVIEW_RULE_REASON: notifier.WEBVIEW_RULE_ADDRS}, actual
        )
    ...
    def test_make_feature_changes_email__new(self, mock_f_e_b):
        ...
        self.assertEqual(
            'new feature: feature one', feature_owner_task['subject']
        )
        self.assertIn('mock body html', feature_owner_task['html'])
        self.assertIn(
            '<li>You are listed as an owner of this feature</li>',
            feature_owner_task['html'],
        )

        # Notification to feature editor.
        self.assertEqual(
            'new feature: feature one', feature_editor_task['subject']
        )
        ...
        self.assertEqual(
            'feature_editor@example.com', feature_editor_task['to']
        )
        ...
        self.assertIn(
            '<li>You are a devrel contact for this feature.</li>',
            devrel_task['html'],
        )
        ...
        self.assertIn(
            "<li>You are CC'd on this feature</li>", feature_cc_task['html']
        )
        self.assertEqual('cc@example.com', feature_cc_task['to'])

        # Notification to component owner.
        self.assertEqual(
            'new feature: feature one', component_owner_task['subject']
        )  # noqa: E501
        ...
        self.assertIn(
            "<li>You are an owner of this feature's component</li>\n"
            '<li>You are listed as an editor of this feature</li>',
        ...
        self.assertIn(
            '<li>You are watching all feature changes</li>',
            watcher_task['html'],
        )
    ...
    def test_make_feature_changes_email__update(self, mock_f_e_b):
        ...
        self.assertEqual('feature_owner@example.com', feature_owner_task['to'])
        self.assertEqual(
            'updated feature: feature one', feature_owner_task['subject']
        )
        self.assertIn('mock body html', feature_owner_task['html'])
        self.assertIn(
            '<li>You are listed as an owner of this feature</li>',
        ...
        # Notification to feature editor.
        self.assertEqual(
            'updated feature: feature one', feature_editor_task['subject']
        )
        self.assertIn('mock body html', feature_editor_task['html'])
        self.assertIn(
            '<li>You are listed as an editor of this feature</li>',
        ...
        self.assertEqual(
            'updated feature: feature one', feature_cc_task['subject']
        )
        self.assertIn('mock body html', feature_cc_task['html'])
        self.assertIn(
            "<li>You are CC'd on this feature</li>", feature_cc_task['html']
        ...
        # Notification to component owner.
        self.assertEqual(
            'updated feature: feature one', component_owner_task['subject']
        )
        self.assertIn('mock body html', component_owner_task['html'])
        # Component owner is also a feature editor and should have both reasons.
        self.assertIn(
        ...
        # Notification to feature change watcher.
        self.assertEqual(
            'updated feature: feature one', watcher_task['subject']
        )
        self.assertIn('mock body html', watcher_task['html'])
        self.assertIn(
            '<li>You are watching all feature changes</li>',
    ...
    def test_make_feature_changes_email__starrer(self, mock_f_e_b):
        ...
        self.assertEqual(
            'updated feature: feature one', starrer_task['subject']
        )
        self.assertIn('mock body html', starrer_task['html'])
        self.assertIn('<li>You starred this feature</li>', starrer_task['html'])
    ...
    def test_make_feature_changes_email__starrer_unsubscribed(self, mock_f_e_b):
        ...
        starrer_2_pref = UserPref(
            email='starrer_2@example.com', notify_as_starrer=False
        )
        starrer_2_pref.put()
        notifier.FeatureStar.set_star(
...
class FeatureCommentHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_new_comments_email__unassigned(
        ...
        (
            ...
            feature_editor_task,
            feature_owner_task,
            feature_editor_task_2,
        ) = actual_tasks  # noqa: E501

        self.assertEqual(
            'New comments for feature: feature one', review_task_1['subject']
        )  # noqa: E501
        self.assertIn('mock body html', review_task_1['html'])
        self.assertIn(
            '<li>You are a reviewer for this type of gate</li>',
            review_task_1['html'],
        )
        ...
        self.assertEqual('feature_owner@example.com', feature_owner_task['to'])
        self.assertEqual(
            'New comments for feature: feature one',
        ...
        # Notification to devrel to feature changes.
        self.assertEqual(
            'New comments for feature: feature one', devrel_task['subject']
        )  # noqa: E501
        self.assertIn('mock body html', devrel_task['html'])
        self.assertIn(
            '<li>You are a devrel contact for this feature.</li>',
        ...
        # Notification to user CC'd on feature changes.
        self.assertEqual(
            'New comments for feature: feature one', feature_cc_task['subject']
        )
        self.assertIn('mock body html', feature_cc_task['html'])
        self.assertIn(
            "<li>You are CC'd on this feature</li>", feature_cc_task['html']
        ...
        mock_f_e_b.assert_called_once_with(
            self.handler.EMAIL_TEMPLATE_PATH,
            self.fe_1,
            [],
    ...
    def test_make_new_comments_email__assigned(self, mock_f_e_b):
        ...
        gate_1 = Gate(
            ...
            stage_id=123,
            state=0,
            assignee_emails=['approver3@example.com'],
        )
        gate_1.put()
        ...
        self.assertIn('mock body html', review_task_1['html'])
        self.assertIn(
            '<li>This review is assigned to you</li>', review_task_1['html']
        )
...
class FeatureReviewHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_review_requests_email__unassigned(
        ...
        # Notification to feature change watcher.
        self.assertEqual(
            'Review Request for feature: feature one', review_task_1['subject']
        )  # noqa: E501
        self.assertIn('mock body html', review_task_1['html'])
        self.assertIn(
            '<li>You are a reviewer for this type of gate</li>',
        ...
        # Notification to feature change watcher.
        self.assertEqual(
            'Review Request for feature: feature one', review_task_2['subject']
        )  # noqa: E501
...
class ReviewAssignementHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_review_assignment_email(self, mock_f_e_b):
        ...
        self.assertEqual(
            'Review assigned for feature: feature one', review_task_1['subject']
        )
        self.assertIn('mock body html', review_task_1['html'])
        self.assertIn(
            '<li>The review is now assigned to you</li>', review_task_1['html']
        )
        ...
        self.assertEqual(
            'Review assigned for feature: feature one', review_task_2['subject']
        )
        self.assertIn('mock body html', review_task_2['html'])
        self.assertIn(
            '<li>The review was previously assigned to you</li>',
            review_task_2['html'],
        )
...
class FeatureStarTest(testing_config.CustomTestCase):
    ...
    def test_get_feature_starrers__some_starrers(self):
        ...
        actual = notifier.FeatureStar.get_feature_starrers(feature_1_id)
        self.assertCountEqual(
            [user_1_email, user_2_email], [au.email for au in actual]
        )
...
class NotifyInactiveUsersHandlerTest(testing_config.CustomTestCase):
    ...
    def setUp(self):
        ...
        really_inactive_user = AppUser(
            created=datetime(2020, 10, 1),
            email='really_inactive_user@example.com',
            is_admin=False,
            is_site_editor=False,
            last_visit=datetime(2022, 10, 1),
        ...
        active_site_editor = AppUser(
            created=datetime(2020, 10, 1),
            email='active_site_editor@example.com',
            is_admin=False,
            is_site_editor=True,
            last_visit=datetime(2023, 7, 30),
        )
        active_site_editor.put()

        inactive_site_editor = AppUser(
            created=datetime(2020, 10, 1),
            email='inactive_site_editor@example.com',
            is_admin=False,
            is_site_editor=True,
            last_visit=datetime(2023, 2, 9),
        )
    ...
    def test_determine_users_to_notify(self):
        ...
        with test_app.app_context():
            ...
            result = inactive_notifier.get_template_data(
                now=datetime(2023, 9, 1)
            )
        expected = (
            '1 users notified of inactivity.\n'
...
class OTExtendedHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_extended_request_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_extended_request_email.html')  # noqa: E501
            self.assertEqual(
                email_task['html'],
...
class OTExtensionApprovedHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_extension_approved_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_extension_approved_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
                (
...
class OTActivatedHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_activated_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_activated_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
                'Example Trial origin trial is now available',
            )
            self.assertEqual(
                email_task['html'], TESTDATA['test_make_activated_email.html']
            )
...
class OTCreationApprovedHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_creation_processed_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_creation_approved_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
...
class OTCreationRequestFailedHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_creation_request_failed_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_creation_request_failed_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
...
class OTActivationFailedHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_activation_failed_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_activation_failed_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
...
class IntentToBlinkDevHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_intent_post_email(self):
        ...
        json_data = {
            ...
            'default_url': (
                f'https://chromestatus.com/feature/{self.feature_1_id}'
        ...
        with test_app.app_context():
            ...
            self.assertEqual(
                email_task['subject'], 'Intent to Experiment: feature one'
            )
            self.assertEqual(
                email_task['html'], TESTDATA['test_make_intent_email.html']
            )
...
class OTEndingNextReleaseReminderHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_ending_next_release_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_ending_next_release_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
...
class OTEndingThisReleaseReminderHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_ending_this_release_email(self):
        ...
        with test_app.app_context():
            ...
            email_task = handler.build_email(body_data, self.contacts)
            # TESTDATA.make_golden(email_task['html'], 'test_make_ending_this_release_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
...
class OTBetaAvailabilityReminderHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_beta_availability_email(self):
        ...
        with test_app.app_context():
            ...
            email_task = handler.build_email(body_data, self.contacts)
            # TESTDATA.make_golden(email_task['html'], 'test_make_beta_availability_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
...
class OTFirstBranchReminderHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_first_branch_email(self):
        ...
        with test_app.app_context():
            ...
            email_task = handler.build_email(body_data, self.contacts)
            # TESTDATA.make_golden(email_task['html'], 'test_make_first_branch_email.html')  # noqa: E501
            self.assertEqual(
...
class OTLastBranchReminderHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_last_branch_email(self):
        ...
        with test_app.app_context():
            ...
            email_task = handler.build_email(body_data, self.contacts)
            # TESTDATA.make_golden(email_task['html'], 'test_make_last_branch_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
                'Some feature origin trial has branched for its last release',
            )
            self.assertEqual(
                email_task['html'], TESTDATA['test_make_last_branch_email.html']
            )
...
class OTAutomatedProcessEmailHandlerTest(testing_config.CustomTestCase):
    ...
    def test_make_ot_process_email(self):
        ...
        with test_app.app_context():
            ...
            # TESTDATA.make_golden(email_task['html'], 'test_make_ot_process_email.html')  # noqa: E501
            self.assertEqual(
                email_task['subject'],
                'Origin trials automated process reminder just ran',
            )
            self.assertEqual(
                email_task['html'], TESTDATA['test_make_ot_process_email.html']
            )
...
class FunctionsTest(testing_config.CustomTestCase):
    ...
    def test_get_thread_id__normal(self):
        ...
        self.assertEqual(
            '456xxx=yyy@mail.gmail.com', notifier.get_thread_id(self.ot_stage)
        )
        self.assertEqual(None, notifier.get_thread_id(self.ship_stage))
