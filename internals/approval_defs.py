...
"""Definitions and configuration for feature review gates defines the various
types of approval gates (e.g., Intent to Ship, Privacy Review), their rules,
required approvers, and Service Level Objectives (SLOs).
"""
...
import settings
from framework import permissions, rediscache
from internals import core_enums, slo
...
ENTERPRISE_APPROVERS = [
    ...
    'carolinasaluz@google.com',
    'crushworth@google.com',
    'sirishsingh@google.com',
]
...
SecurityOriginTrialApproval = GateInfo(
    'Security OT Review',
    'Security OT Review',
    core_enums.GATE_SECURITY_ORIGIN_TRIAL,
    ONE_LGTM,
...
EnterpriseShipApproval = GateInfo(
    ...
    ONE_LGTM,
    approvers=ENTERPRISE_APPROVERS,
...
EnterprisePlanApproval = GateInfo(
    ...
    'Enterprise Deprecation Plan Review',
    core_enums.GATE_ENTERPRISE_PLAN,
    ONE_LGTM,
    approvers=ENTERPRISE_APPROVERS,
    team_name='Enterprise',
)
...
DebuggabilityOriginTrialApproval = GateInfo(
    ...
    'Debuggability OT Review',
    core_enums.GATE_DEBUGGABILITY_ORIGIN_TRIAL,
    ONE_LGTM,
    approvers=DEBUGGABILITY_APPROVERS,
...
DebuggabilityPlanApproval = GateInfo(
    ...
    'Debuggability Deprecation Plan Review',
    core_enums.GATE_DEBUGGABILITY_PLAN,
    ONE_LGTM,
    approvers=DEBUGGABILITY_APPROVERS,
    team_name='Debuggability',
    escalation_email='devtools-dev@chromium.org',
...
APPROVAL_FIELDS_BY_ID = {
    ...
    for afd in [
        PrototypeApproval,
        ExperimentApproval,
        ExtendExperimentApproval,
        ShipApproval,
        PlanApproval,
        PrivacyOriginTrialApproval,
        PrivacyShipApproval,
        SecurityOriginTrialApproval,
        SecurityShipApproval,
        EnterpriseShipApproval,
        EnterprisePlanApproval,
        DebuggabilityOriginTrialApproval,
        DebuggabilityShipApproval,
...
def fetch_owners(url) -> list[str]:
    ...
    if response.status_code == 200:
        ...
        logging.error(
            'Got response %s', repr(response)[: settings.MAX_LOG_LINE]
        )
        if owners_file:
...
def auto_assign_reviewer(gate):
    """If a previous review was assigned, use the same reviewer if this gate
    has a reviewer rotation, use the current on-call user.
    """  # noqa: D205
    afd = APPROVAL_FIELDS_BY_ID[gate.gate_type]

    all_gates: list[Gate] = Gate.query(
        Gate.feature_id == gate.feature_id
    ).fetch()
    for other_gate in all_gates:
        other_afd = APPROVAL_FIELDS_BY_ID[other_gate.gate_type]
        if (
            other_gate.key.integer_id() != gate.key.integer_id()
            and other_afd.team_name == afd.team_name
            and other_afd.rule == afd.rule
            and other_gate.assignee_emails
        ):
    ...
    try:
        logging.info(
            'response.content is:\n%s',
            response.content[: settings.MAX_LOG_LINE],
        )
...
def fields_approvable_by(user):
    ...
    approvable_ids = {
        gate_type
        for gate_type in APPROVAL_FIELDS_BY_ID
...
def set_vote(
    ...
    if gate_id is None and gate_type is not None:
        ...
        if not gate:
            logging.warning(
                'Gate entity not found for the given feature. Cannot set vote.'
            )
            return None
    ...
    if gate:
        ...
        double_votes = [
            v for v in votes if v.set_by == set_by_email and v.set_on < now
        ]
...
def get_gate_by_type(feature_id: int, gate_type: int):
    ...
    gates: list[Gate] = Gate.query(
        Gate.feature_id == feature_id, Gate.gate_type == gate_type
    ).fetch()
    if len(gates) == 0:
        return None
    return gates[0]
