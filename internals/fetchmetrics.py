...
import settings
from framework import basehandlers, rediscache, utils
from internals import metrics_models, user_models
...
class UmaQuery(object):
    ...
    def _FetchData(self, date):
        ...
        if not result or result.status_code != 200:
            logging.error(
                'Unable to retrieve UMA data from %s. Error: %s'
        ...
        logging.info(
            'full response: %r', full_response_content[: settings.MAX_LOG_LINE]
        )
    ...
    def get_template_data(self, **kwargs):  # noqa: D417
        ...
        if date_str:
            try:
                ...
                specified_day = datetime.datetime.strptime(
                    date_str, '%Y%m%d'
                ).date()
            ...
            days = [
                today - datetime.timedelta(days_ago)
...
class HistogramsHandler(basehandlers.FlaskHandler):
    ...
    def _SaveData(self, bucket_id, property_name, model_class, all_existing):
        ...
        found = False
        relevant_existing = [
            ex for ex in all_existing if ex.bucket_id == bucket_id
        ]
        ...
        if not found:
            new_entity = model_class(
                bucket_id=bucket_id, property_name=property_name
            )
            new_entity.put()
    ...
    def get_template_data(self, **kwargs):
        ...
        if response.status_code != 200:
            logging.error(
                'Unable to retrieve chromium histograms mapping file.'
            )
            self.abort(500)
