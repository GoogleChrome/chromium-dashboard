...
def store_attachment(
    ...
    attachment = Attachment(
        feature_id=feature_id, content=content, mime_type=mime_type
    )
    attachment.put()
...
def check_attachment_size(content: bytes):
    ...
    if len(content) > settings.MAX_ATTACHMENT_SIZE:
        raise AttachmentTooLarge(
            'Attachment size %r is too large' % len(content)
        )
...
def get_attachment(feature_id: int, attachment_id: int) -> Attachment | None:
    """Return attachment, if feature_id  matches, and attachment is not deleted."""  # noqa: E501
    attachment = Attachment.get_by_id(attachment_id)
    if (
        attachment
        and attachment.feature_id == feature_id
        and not attachment.is_deleted
    ):
        return attachment
...
def delete_orphan_attachments(feature_id: int, new_value: str) -> None:
    ...
    attachments = Attachment.query(Attachment.feature_id == feature_id).fetch()
...
def get_thumbnail(attachment_id: int) -> Thumbnail | None:
    ...
    thumbnail = Thumbnail.query(Thumbnail.attachment_id == attachment_id).get()
    return thumbnail
