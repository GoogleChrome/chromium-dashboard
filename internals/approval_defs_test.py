...
class FetchOwnersTest(testing_config.CustomTestCase):
    ...
    def test__error__use_ndb(self, mock_get, mock_err):
        ...
        OwnersFile(
            ...
            raw_content=encoded,
            created_on=datetime.datetime(2022, 1, 1),
        ).put()
        mock_get.return_value = testing_config.Blank(status_code=404)
...
class CalcGateStateTest(testing_config.CustomTestCase):
    ...
    def do_calc(self, *vote_states):
        ...
        actual_3 = approval_defs._calc_gate_state(
            votes, approval_defs.THREE_LGTM
        )
    ...
    def test_no_votes_yet(self):
        ...
        self.assertEqual(('preparing', 'preparing'), self.do_calc())

    def test_just_requested(self):
        """The user has requested a review."""
        self.assertEqual(
            ('review_requested', 'review_requested'), self.do_calc(RR)
        )
    ...
    def test_request_three_approved(self):
        ...
        self.assertEqual(('approved', 'approved'), self.do_calc(RR, AP, AP, AP))

    def test_request_needs_work(self):
        """The user has requested a review and a reviewer said it needs work."""
        self.assertEqual(('needs_work', 'needs_work'), self.do_calc(RR, NW))
        self.assertEqual(('needs_work', 'needs_work'), self.do_calc(RR, RS, NW))
        self.assertEqual(('needs_work', 'needs_work'), self.do_calc(RR, NW, NW))

    def test_request_internal_review(self):
        """Owner requested a review and a reviewer opts for internal review."""
        self.assertEqual(
            ('internal_review', 'internal_review'), self.do_calc(RR, IR)
        )
        self.assertEqual(
            ('internal_review', 'internal_review'), self.do_calc(RR, RS, IR)
        )
        self.assertEqual(
            ('internal_review', 'internal_review'), self.do_calc(RR, NW, IR)
        )

    def test_request_disagreement(self):
        """Reviewers may have different opinions, needed LGTMs counted."""
        self.assertEqual(('approved', 'needs_work'), self.do_calc(RR, AP, NW))
        self.assertEqual(('approved', 'needs_work'), self.do_calc(RR, NW, AP))
        self.assertEqual(('approved', 'review_requested'), self.do_calc(RR, AP))
        self.assertEqual(
            ('approved', 'review_requested'), self.do_calc(RR, AP, AP)
        )
        self.assertEqual(
            ('approved', 'review_started'), self.do_calc(RR, AP, AP, RS)
        )
        self.assertEqual(
            ('approved', 'needs_work'), self.do_calc(RR, AP, NW, AP)
        )

    def test_ping(self):
        """Feature owner re-requested review after feedback or waiting."""
        self.assertEqual(
            ('review_requested', 'review_requested'), self.do_calc(NW, RR)
        )
        self.assertEqual(
            ('review_requested', 'review_requested'), self.do_calc(DN, RR)
        )
        self.assertEqual(
            ('review_requested', 'review_requested'), self.do_calc(RS, RR)
        )
        self.assertEqual(
    ...
    def test_ping_pong(self):
        ...
        self.assertEqual(
            ('review_started', 'review_started'), self.do_calc(NW, RR, RS)
        )
        self.assertEqual(
            ('approved', 'review_requested'), self.do_calc(DN, RR, AP)
        )
        self.assertEqual(
            ('approved', 'needs_work'), self.do_calc(RS, RR, AP, NW)
        )
        self.assertEqual(('approved', 'approved'), self.do_calc(AP, AP, RS, AP))

    def test_self_cert_stands(self):
        """A feature owner self-certified and no one disagrees."""
        self.assertEqual(
            ('na (self-certified)', 'preparing'), self.do_calc(NA_SELF)
        )
        self.assertEqual(('na', 'review_requested'), self.do_calc(NA_SELF, NA))
        self.assertEqual(
            ('approved', 'review_requested'), self.do_calc(NA_SELF, AP)
        )

    def test_self_cert_reversed(self):
        """A feature owner self-certified but a reviewer disagreed."""
        self.assertEqual(
            ('needs_work', 'needs_work'), self.do_calc(NA_SELF, NW)
        )
        self.assertEqual(
            ('needs_work', 'needs_work'), self.do_calc(NA_SELF, NR, NW, NR)
        )
        self.assertEqual(
            ('review_started', 'review_started'), self.do_calc(NA_SELF, RS)
        )
        self.assertEqual(
            ('internal_review', 'internal_review'), self.do_calc(NA_SELF, IR)
        )
        self.assertEqual(('denied', 'denied'), self.do_calc(NA_SELF, DN))

    def test_self_cert_verified(self):
        """A feature owner self-certified and a reviewer verfied."""
        self.assertEqual(
            ('na (verified)', 'preparing'), self.do_calc(NA_SELF, NA_VERIFIED)
        )
        self.assertEqual(
            ('na (verified)', 'preparing'),
            self.do_calc(NA_SELF, NR, NA_VERIFIED, NR),
        )
        self.assertEqual(
            ('na (verified)', 'needs_work'),
            self.do_calc(NA_SELF, RS, NW, NA_VERIFIED),
        )
        self.assertEqual(
            ('na (verified)', 'review_started'),
    ...
    def setUp(self):
        ...
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.APPROVED,
                set_on=datetime.datetime(2020, 1, 1),
                set_by='user1@example.com',
            )
        )
        self.votes.append(
            Vote(
        ...
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.REVIEW_REQUESTED,
                set_on=datetime.datetime(2020, 2, 1),
        ...
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.REVIEW_REQUESTED,
                set_on=datetime.datetime(2020, 3, 1),
        ...
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.REVIEW_REQUESTED,
                set_on=datetime.datetime(2020, 1, 2),
        ...
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.REVIEW_REQUESTED,
                set_on=datetime.datetime(2020, 7, 1),
        ...
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.APPROVED,
                set_on=datetime.datetime(2020, 4, 10),
                set_by='user8@example.com',
            )
        )
        self.votes.append(
            Vote(
                ...
                set_on=datetime.datetime(2020, 1, 15),
                set_by='user9@example.com',
            )
        )
        self.votes.append(
            Vote(
                ...
                set_on=datetime.datetime(2020, 8, 23),
                set_by='user10@example.com',
            )
        )
        self.votes.append(
            Vote(
                ...
                gate_id=gate_id,
                state=Vote.APPROVED,
                set_on=datetime.datetime(2021, 1, 1),
                set_by='user11@example.com',
            )
        )
