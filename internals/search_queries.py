...
from internals.search_fulltext import FULLTEXT_FIELDS, search_fulltext
...
def build_filter(
    ...
    if len(val_list) > 1:
        if operator == '=':
            ...
            raise ValueError(
                'Quick-OR is not supported for operator: %r' % operator
            )
...
def single_field_query_async(
    ...
    if field_name in COMPLEX_FIELDS:
        ...
        if core_enums.is_enum_field(field_name):
            ...
            enum_val_list = []
            for val in val_list:
                enum_val = core_enums.convert_enum_string_to_int(
                    field_name, val
                )
                if enum_val < 0:
...
def sorted_by_pending_request_date(descending: bool) -> Future:
    ...
    return _sorted_by_joined_model(
        Gate, Gate.state.IN(Gate.PENDING_STATES), descending, Gate.requested_on
    )
...
def sorted_by_review_date(descending: bool) -> Future:
    ...
    return _sorted_by_joined_model(
        Vote, Vote.state.IN(Gate.FINAL_STATES), descending, Vote.set_on
    )
...
QUERIABLE_FIELDS: dict[str, Property] = {
    ...
    'initial_public_proposal_url': FeatureEntry.initial_public_proposal_url,
    ...
    'standards.anticipated_spec_changes': FeatureEntry.anticipated_spec_changes,
    ...
    for field_name in [
        ...
        disjunction.append(
            ndb.AND(
                ...
                build_filter(
                    STAGE_QUERIABLE_FIELDS[field_name], operator, val_list
                ),
            )
        )
