...
class ActivityTest(testing_config.CustomTestCase):
    ...
    def test_activities_created__empty_list_not_created(self):
        """No amendment should be logged if the value moved from None to empty list."""  # noqa: E501
    ...
    def test_vote_changes_activities__needs_work_note(self, mock_task_helpers):
        ...
        prop_change = {
            ...
            'old_val': 'na',
            'new_val': 'needs_work',
            'note': 'Feature owners must press the "Re-request review" button after requested changes have been completed.',  # noqa: E501
        }
        expected_params = {
            'changes': [prop_change],
            'is_update': True,
...
class NotifierHelpersTest(testing_config.CustomTestCase):
    def setUp(self):
        ...
        feature_id = self.feature_1.key.integer_id()
        self.ot_stage = Stage(id=123, feature_id=feature_id, stage_type=150)
        self.extension_stage = Stage(
            id=456, feature_id=feature_id, stage_type=151
        )
        self.ot_stage.put()
