...
def build_email_tasks(
    ...
    for fe, mstone in features_to_notify:
        ...
        recipients = choose_email_recipients(
            fe, is_escalated, is_accuracy_email()
        )
...
class FeatureAccuracyHandler(AbstractReminderHandler):
    ...
    def prefilter_features(
        ...
        prefiltered_features = [
            ...
            if (
                feature.accurate_as_of is None
                or feature.accurate_as_of + self.ACCURACY_GRACE_PERIOD < now
            )
        ]
    ...
    def changes_after_sending_notifications(
        self, notified_features: list[tuple[FeatureEntry, int]]
    ) -> None:
...
class SLOOverdueHandler(basehandlers.FlaskHandler):
    ...
    def get_template_data(self, **kwargs):
        ...
        (
            ...
            newly_overdue_resolve,
            long_overdue_resolve,
            relevant_features,
        ...
        email_tasks = (
            newly_initial_email_tasks
            + long_initial_email_tasks
            + newly_resolve_email_tasks
            + long_resolve_email_tasks
        )
        notifier.send_emails(email_tasks)
        ...
        if len(email_tasks):
            recipients = '\n'.join(
                sorted(
                    list(
                        set(
                            [task['to'] for task in email_tasks if 'to' in task]
                        )
                    )
                )
            )
    ...
    def get_overdue_gates_and_features(self):
        ...
        for g in active_gates:
            ...
            slo_limit = (
                appr_def.slo_initial_response
                if appr_def
                else approval_defs.DEFAULT_SLO_LIMIT
            )
            slo_resolve_limit = (
                appr_def.slo_resolve
                if appr_def
                else approval_defs.DEFAULT_SLO_RESOLVE_LIMIT
            )
            initial_remaining = slo.remaining_days(g.requested_on, slo_limit)
            resolve_remaining = slo.remaining_days(
                g.requested_on, slo_resolve_limit
            )
    ...
    def build_gate_email_tasks(
        ...
        for gate in gates_to_notify:
            ...
            gate_url = (
                settings.SITE_URL + f'feature/{feature_id}?gate={gate_id}'
            )
            ...
            if is_escalated:
                if EMAIL_SUBJECT_PREFIX in subject:
                    subject = subject.replace(
                        EMAIL_SUBJECT_PREFIX, 'Escalation request'
                    )
                else:
                    subject = f'ESCALATED: {subject}'
