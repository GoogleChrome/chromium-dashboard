...
"""NDB models for feature review gates, votes, activities, and API owners."""
...
class OwnersFile(ndb.Model):
    ...
    def add_owner_file(self):
        ...
        ndb.delete_multi(
            OwnersFile.query(OwnersFile.url == self.url).fetch(keys_only=True)
        )
        return self.put()
    ...
    def get_raw_owner_file(cls, url) -> OwnersFile | None:
        ...
        if not owners_file_list:
            logging.info(
                'API_OWNERS content does not exist for URL %s.' % (url)
            )
            return None
