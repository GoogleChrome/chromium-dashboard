...
def get_stage_info_for_templates(fe: FeatureEntry) -> StageTemplateInfo:
    ...
    for s in Stage.query(Stage.feature_id == id):
        ...
        if (
            s.stage_type != proto_stage_type
            and s.stage_type != dt_stage_type
            and s.stage_type != ot_stage_type
            and s.stage_type != extension_stage_type
            and s.stage_type != ship_stage_type
            and s.stage_type != enterprise_stage_type
        ):
            continue
...
def look_up_year(milestone: int) -> int:
    ...
    if release_info and 'final_beta' in release_info:
        ...
        shipping_date = datetime.strptime(
            shipping_date_str, utils.CHROMIUM_SCHEDULE_DATE_FORMAT
        ).date()
...
def get_all_shipping_stages_with_milestones(
    ...
    if feature_id:
        shipping_query = shipping_query.filter(Stage.feature_id == feature_id)
    ...
    shipping_stages_with_milestones = [
        ...
        if (
            ...
            and (
                ...
                or stage.milestones.android_first
                or stage.milestones.ios_first
                or stage.milestones.webview_first
            )
        )
    ]
