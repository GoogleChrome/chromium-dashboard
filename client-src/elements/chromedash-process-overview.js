...
export class ChromedashProcessOverview extends LitElement {
  ...
  inFinalStage(stage) {
    ...
    return (
      FINAL_STAGES.includes(this.feature.intent_stage) &&
  ...
  isStartableStage(stage) {
    ...
    const featureStageIndex = stageOrder.indexOf(this.feature.intent_stage_int);
    return (
      viewedIncomingStageIndex <= featureStageIndex &&
  ...
  renderProgressItem(stage, feStage, item) {
    ...
    if (!this.progress.hasOwnProperty(item.name)) {
      return html` <div class="pending">${item.name} ${editEl}</div>`;
    }

    const progressValue = this.progress[item.name];
    if (
      progressValue.startsWith('http://') ||
      progressValue.startsWith('https://')
    ) {
      return html` <div class="done">
        <a target="_blank" href="${progressValue}"> ${item.name} </a>
        ${editEl}
      </div>`;
    }

    return html` <div class="done">${item.name} ${editEl}</div>`;
  }
  ...
  renderProcessStage(featureId, feStage) {
    ...
    return html` <tr class="${isActive ? 'active' : ''}">
      <td>
      ...
      <td>${button} ${this.renderActions(processStage, feStage)}</td>
    </tr>`;
  }
      ...
      <div style="position: relative">
        <table>
          ...
          ${this.feature.stages.map(feStage =>
            this.renderProcessStage(this.feature.id, feStage)
          )}
          <tr>
        ...
        ${Object.keys(this.progress).length
          ? html` <chromedash-callout
              cue="progress-checkmarks"
              targetid="progress-header"
              signedin
              .dismissedCues=${this.dismissedCues}
            >
              Progress checkmarks appear in this column as you fill in fields of
              the feature entry. However, you may start the next stage
              regardless of checkmarks.
            </chromedash-callout>`
          : nothing}
      </div>
    `;
