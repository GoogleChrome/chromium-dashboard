...
export class ChromedashBulkEditPage extends LitElement {
  static get styles() {
    return [
      ...
      ...FORM_STYLES,
      css`
      section { margin: var(--content-padding); }
      `,
    ];
  }
  ...
  selectItems() {
    ...
    const chromestatusUrlIndex: number = headerCells.indexOf('Chrome Status Entry');
    this.items = [];
  ...
  getFileAndParse() {
    var fileField: HTMLInputElement = this.shadowRoot!.querySelector('#id_file_form_field') as HTMLInputElement;
  ...
  renderPreview() {
    if (this.items.length == 0) {
      return html`
        <p>Select a CSV file to see the preview</p>
      `;
    }
    return html`
