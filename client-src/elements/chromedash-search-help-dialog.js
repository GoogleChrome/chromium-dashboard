...
export async function openSearchHelpDialog() {
  if (!searchHelpDialogEl) {
    searchHelpDialogEl = document.createElement('chromedash-search-help-dialog');
...
export class ChromedashSearchHelpDialog extends LitElement {
  ...
  renderCommonSearchExamples() {
    return html`
      ...
      <table class="data-table">
        ${this.renderExampleRow(['memory', 'memory pool', 'memory -pool'],
        ...
        ${this.renderExampleRow(['owner:user@example.com'],
  ...
  renderSearchSyntax() {
    return html`
      ...
      <p>A search query consists of a series of terms that are separated by
      spaces. Terms can be single words or conditions.</p>
      ...
      Search terms are implicitly joined together by a logical-AND.
      However, you may type the keyword <code>OR</code> between terms as
      a logical-OR operator.  We do not support parenthesis in queries.
      </p>
  ...
  renderDialogContent() {
    return html`
      <div id="dialog-content">
        ${this.renderCommonSearchExamples()}
        ${this.renderSearchSyntax()}
...
customElements.define(
  'chromedash-search-help-dialog', ChromedashSearchHelpDialog);
