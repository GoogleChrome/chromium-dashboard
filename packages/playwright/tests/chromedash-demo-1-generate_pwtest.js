...
test('Demo 1: AI Summary Generation & Draft Status Badge', async ({ page, request }) => {
  ...
  test.setTimeout(120000); // Give the live Gemini Agent ample time for multi-turn tool execution!
  ...
  console.log('[Demo 1] Step 5b: Expanding AI Generation Progress Details');
  const detailsBtn = card.locator('.details-toggle-btn');
  await expect(detailsBtn).toBeVisible({ timeout: 20000 });
  await detailsBtn.click();
  await page.waitForTimeout(1000); // Pause to let viewer see console expand

  console.log('[Demo 1] Step 6: Waiting for the Draft Available badge');
  const suggestionBadge = card.locator('sl-tag[variant="success"]');
  // Dynamic timeout up to 75 seconds for real live Gemini generation with tool lookups
