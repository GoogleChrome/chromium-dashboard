...
test('add an origin trial stage', async ({page}) => {
  ...
  await test.step('Open the Add Stage dialog', async () => {
    ...
    const incubatingPanel = page.locator('sl-details', {
      hasText: 'Start incubating',
    });
  ...
  await test.step('Select Origin Trial stage and verify dialog', async () => {
    ...
    // Locate the option visible in the dropdown.
    const originTrialOption = page
    ...
    // Ensure the mask selector is specific enough.
    await expect(page).toHaveScreenshot(
      'create-origin-trial-stage-dialog.png',
      {
  ...
  await test.step('Verify Origin Trial panels', async () => {
    // Define panels.
    const otPanel1 = page
      .locator('sl-details', {hasText: 'Origin Trial'})
      .first();
    const otPanel2 = page
      .locator('sl-details', {hasText: 'Origin Trial 2'})
      .first();
    ...
    const prepareToShip = page.locator('sl-details', {
      hasText: 'Prepare to ship',
    });
