...
test('test semantic checks', async ({page}) => {
  ...
  const devTrialDesktopInput = page.locator(
  ...
  const devTrailDesktopMilestoneLocator = page.locator(
    'chromedash-form-field[name="dt_milestone_desktop_start"]'
  );
  await expect(
    devTrailDesktopMilestoneLocator.locator('.check-warning')
  ).toHaveCount(0);
  ...
  const shippedAndroidInput = page.locator(
    'input[name="shipped_android_milestone"]'
  );
  ...
  // Check that there is no error now for the dev trail milestone field
  const shippedDesktopMilestoneLocator = page.locator(
    'chromedash-form-field[name="shipped_milestone"]'
  );
  await expect(
    shippedDesktopMilestoneLocator.locator('.check-warning')
  ).toHaveCount(0);
});
