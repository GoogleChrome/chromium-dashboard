...
async function gotoEditAllPage(page) {
  ...
  await expect(page.locator('chromedash-form-table')).toBeVisible({ timeout: 30000 });
}
...
test('test semantic checks', async ({ page }) => {
  ...
  await test.step('Verify Dev Trial Milestone (Valid)', async () => {
    const devTrialInput = page.locator('input[name="dt_milestone_desktop_start"]');
    await devTrialInput.fill('100');
    await devTrialInput.blur();

    // Ensure no warning appears
    const devTrialField = page.locator('chromedash-form-field[name="dt_milestone_desktop_start"]');
  ...
  await test.step('Trigger Shipped Milestone Conflict', async () => {
    ...
    const shippedField = page.locator('chromedash-form-field[name="shipped_milestone"]');
