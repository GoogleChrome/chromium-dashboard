...
class WPTContents:
    ...
    test_to_dependencies_map: dict[Path, set[Path]] = field(
        ...
        def wrapper(*args, **kwargs):
            ...
            while _tries > 0:
                try:
                    ...
                    if _tries == 0:
                        logging.error(
                            'Exceeded maximum number of retries for %s.',
                            func.__name__,
                        )
                        raise
                    trace_str = traceback.format_exc()
                    logging.warning(
                        'Retrying %s due to Exception: %s',
                        func.__name__,
                        trace_str[: settings.MAX_LOG_LINE],
                    )
                    time.sleep(_delay)
...
async def get_mixed_wpt_contents_async(
    ...
    dir_tasks = [
        asyncio.to_thread(_fetch_dir_listing, u, headers) for u in dir_urls
    ]
    ...
    for furl in additional_file_urls:
        ...
        initial_file_infos.append(
            (f'{WPT_GITHUB_RAW_CONTENTS_URL}{path}', path)
        )
    ...
    if len(files_to_fetch_map) > MAXIMUM_TEST_SUITE_SIZE:
        raise PipelineError(
            MAXIMUM_TEST_SUITE_SIZE_OUTPUT.format(
                num_files=len(files_to_fetch_map)
            )
        )
    ...
    while processing_queue:
        ...
        file_tasks = [
            _fetch_and_pair(fpath, url) for fpath, url in processing_queue
        ]
            ...
            for dep in dependencies:
                ...
                if resolved_path:
                    ...
                    if resolved_path not in visited_paths:
                        ...
                        dep_url = (
                            f'{WPT_GITHUB_RAW_CONTENTS_URL}{resolved_path}'
                        )
    ...
    for test_path in test_contents:
        ...
        while stack:
            ...
            if current_path != test_path:
                if (
                    current_path in dependency_contents
                    or current_path in test_contents
                ):
