...
class ApiCredential(ndb.Model):
    ...
    def select_token_for_api(cls, api_name: str) -> 'ApiCredential':
        ...
        sorted_for_api = sorted(
            all_for_api, key=lambda ac: ac.failure_timestamp
        )
...
def load_ot_api_key():
    ...
    if settings.DEV_MODE or settings.UNIT_TEST_MODE:
        ...
        name = (
            f'{client.secret_path(settings.APP_ID, "OT_API_KEY")}'
            '/versions/latest'
        )
...
def load_github_token():
    ...
    if settings.DEV_MODE or settings.UNIT_TEST_MODE:
        ...
        client = SecretManagerServiceClient()
        name = (
            f'{client.secret_path(settings.APP_ID, "GITHUB_TOKEN")}'
            '/versions/latest'
        )
        response = client.access_secret_version(request={'name': name})
        if response:
...
def load_gemini_api_key() -> None:
    ...
    if settings.DEV_MODE or settings.UNIT_TEST_MODE:
        ...
        client = SecretManagerServiceClient()
        name = (
            f'{client.secret_path(settings.APP_ID, "GEMINI_API_KEY")}'
            '/versions/latest'
        )
        response = client.access_secret_version(request={'name': name})
        if response:
...
def get_ot_support_emails() -> str | None:
    ...
    client = SecretManagerServiceClient()
    name = (
        f'{client.secret_path(settings.APP_ID, "OT_SUPPORT_EMAILS")}'
        '/versions/latest'
    )
    response = client.access_secret_version(request={'name': name})
    if response:
...
def get_ot_data_access_admin_group() -> str | None:
    ...
    client = SecretManagerServiceClient()
    secret_path = client.secret_path(
        settings.APP_ID, 'OT_DATA_ACCESS_ADMIN_GROUP_NAME'
    )
    name = f'{secret_path}/versions/latest'
    response = client.access_secret_version(request={'name': name})
    if response:
        settings.OT_DATA_ACCESS_ADMIN_GROUP_NAME = response.payload.data.decode(
            'UTF-8'
        )
        return settings.OT_DATA_ACCESS_ADMIN_GROUP_NAME
    return None
