...
"""Email formatting and sending utilities handles the formatting, routing, and
sending of outbound emails, including special handling for staging
environments and Cloud Tasks integration.
"""
...
def handle_outbound_mail_task():
    ...
    if (
        settings.SEND_ALL_EMAIL_TO
        and to != settings.REVIEW_COMMENT_MAILING_LIST
    ):
        ...
        if cc:
            ...
            for cc_addr in cc:
                ...
                new_cc.append(
                    settings.CC_ALL_EMAIL_TO
                    % {'user': cc_user, 'domain': cc_domain}
                )
            cc = new_cc
...
def receive(bounce_message):
    ...
    if settings.SEND_EMAIL:
        message.send()
    logging.info(
        'Full parsed original message that was sent: %r',
        bounce_message.original,
    )
    logging.info(
        'Full parsed notification of the bounce: %r',
...
def handle_incoming_mail(addr=None):
    """Handle an incoming email by making a task to examine it this code checks
    some basic properties of the incoming message to make sure     that it
    is worth examining.  Then it puts all the relevent fields into a
    dict and makes a new Cloud Task which is futher processed in python 3
    code.
    """
    ...
    from_addrs = _extract_addrs(
        msg.get_all('x-original-from', [])
    ) or _extract_addrs(msg.get_all('from', []))
    if from_addrs:
...
def add_routes(app):
    ...
    app.add_url_rule(
        '/tasks/outbound-email',
        view_func=handle_outbound_mail_task,
        methods=['POST'],
    )
