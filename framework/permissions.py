...
def _can_view_confidential_feature_formatted(user: User, feature: dict):
    ...
    is_participant = (
        # Basic JSON format
        email in feature.get('owners', [])
        or email in feature.get('editors', [])
        or email == feature.get('creator')
        or
        # Verbose or tiny JSON formats
        email in feature.get('owner_emails', [])
        or email in feature.get('editor_emails', [])
        or email in feature.get('cc_emails', [])
    )
    if is_participant:
        return True
...
def _can_view_confidential_feature(user: User, feature: FeatureEntry):
    ...
    email = user.email()
    if (
        email in feature.owner_emails
        or email in feature.editor_emails
        or email == feature.creator_email
    ):
        return True
...
def can_review_release_notes(user: User) -> bool:
    ...
    return (
        app_user and app_user.is_admin
    ) or user.email() in RELEASE_NOTE_REVIEWERS
...
def can_edit_feature(user: User, feature_id: int) -> bool:
    ...
    return (
        email in feature.owner_emails
        or email in feature.editor_emails
        or (feature.spec_mentor_emails and email in feature.spec_mentor_emails)
        or email == feature.creator_email
    )


def can_review_gate(
    user: User, feature: FeatureEntry, gate: Gate | None, approvers: list[str]
) -> bool:
    ...
    is_assigned = (
        ...
        and user.email() in gate.assignee_emails
    )
...
def _maybe_redirect_to_login(handler_obj) -> flask.Response | dict:
    ...
    if (
        'current_path' in common_data
        and 'loginStatus=False' in common_data['current_path']
    ):
        return {}
...
def _reject_or_proceed(
    handler_obj, handler_method, handler_args, handler_kwargs, perm_function
):
...
def require_admin_site(handler):
    ...
    def check_login(self, *args, **kwargs):
        return _reject_or_proceed(self, handler, args, kwargs, can_admin_site)
...
def require_view_feature(handler):
    ...
    def check_login(self, *args, **kwargs):
        return _reject_or_proceed(self, handler, args, kwargs, can_view_feature)
