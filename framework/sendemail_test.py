...
"""Unit tests for the sendemail module tests parameter parsing and the
construction and sending of emails across different environments (local,
staging, prod).
"""
...
class FunctionTests(testing_config.CustomTestCase):
    def test_get_param__simple(self):
        ...
        with test_app.test_request_context('/test', json={'x': 1}):
            self.assertEqual(1, sendemail.get_param(flask.request, 'x'))
            self.assertEqual(
                None,
...
class BouncedEmailHandlerTest(testing_config.CustomTestCase):
    ...
    def test_receive__user_has_prefs(self, mock_emailmessage_constructor):
        ...
        starrer_3_pref = UserPref(
            email='starrer_3@example.com', notify_as_starrer=False
        )
        starrer_3_pref.put()

        bounce_message = testing_config.Blank(
            original={
                'to': 'starrer_3@example.com',
                'from': 'sender',
                'subject': 'subject',
                'text': 'body',
            },
...
class FunctionTest(testing_config.CustomTestCase):
    def test_extract_addrs(self):
        ...
        header_val = ''
        self.assertEqual([], sendemail._extract_addrs(header_val))
...
class InboundEmailHandlerTest(testing_config.CustomTestCase):
    def test_handle_incoming_mail__wrong_to_addr(self):
        ...
        self.assertEqual({'message': 'Wrong address'}, actual)
    ...
    def test_handle_incoming_mail__too_big(self):
        ...
        self.assertEqual({'message': 'Too big'}, actual)
    ...
    def test_handle_incoming_mail__junk_mail(self, mock_get_incoming_message):
        ...
        for precedence in ['Bulk', 'Junk']:
            msg = MakeMessage(
                HEADER_LINES + [('Precedence', precedence)], 'I am on vacation!'
            )
            ...
            with test_app.test_request_context(
                ...
                actual = sendemail.handle_incoming_mail(
                    settings.INBOUND_EMAIL_ADDR
                )

            self.assertEqual({'message': 'Wrong precedence'}, actual)

    @mock.patch('framework.sendemail.get_incoming_message')
    def test_handle_incoming_mail__unclear_from(
        self, mock_get_incoming_message
    ):
