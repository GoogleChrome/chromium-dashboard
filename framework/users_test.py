...
"""Unit tests for the users module tests the retrieval of the current user,
session management, and handling of mock authentication states during
testing.
"""
...
from framework import users, xsrf
...
class UsersTest(testing_config.CustomTestCase):
    ...
    def test_get_current_user__signed_in_sui(self, mock_validate):
        ...
        with test_app.test_request_context('/any-path'):
            session.clear()
            session['signed_user_info'] = (
                {'email': 'user_333@example.com'},
                'good',
            )
            actual_user = users.get_current_user()
