...
"""Unit tests for the secrets module tests the generation of random keys,
retrieval of cached secrets, and interactions with Google Cloud Secret
Manager.
"""
...
class SecretsFunctionsTest(testing_config.CustomTestCase):
    ...
    def test_load_ot_api_key__cached(self, mock_open, mock_sm_client):
    ...
    @mock.patch('google.cloud.secretmanager.SecretManagerServiceClient')
    def test_load_ot_api_key__prod_mode_secret_exists(
    ...
    @mock.patch('google.cloud.secretmanager.SecretManagerServiceClient')
    @mock.patch('builtins.open')
    def test_load_github_token__cached(self, mock_open, mock_sm_client):
    ...
    def test_load_github_token__prod_mode_secret_fails_raises_runtimeerror(
        ...
        with self.assertRaisesRegex(
            RuntimeError, 'Failed to obtain the GitHub token from secrets.'
        ):
            secrets.load_github_token()
    ...
    @mock.patch('google.cloud.secretmanager.SecretManagerServiceClient')
    @mock.patch('builtins.open')
    def test_load_gemini_api_key__cached(self, mock_open, mock_sm_client):
    ...
    def test_load_gemini_api_key__prod_mode_secret_fails_raises_runtimeerror(
        ...
        with self.assertRaisesRegex(
            RuntimeError, 'Failed to obtain the Gemini API key from secrets.'
        ):
            secrets.load_gemini_api_key()
...
class ApiCredentialTest(testing_config.CustomTestCase):
    def test_select_token_for_api__first_use(self):
        ...
        self.assertEqual(1, len(list(secrets.ApiCredential.query())))
    ...
    def test_select_token_for_api__three_exist(self):
        ...
        self.assertEqual(3, len(list(secrets.ApiCredential.query())))
