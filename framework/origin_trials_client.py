...
def get_trials_list() -> list[dict[str, Any]]:
    ...
    trials_list = [
        asdict(OriginTrialInfo(api_trial))
...
def _send_create_trial_request(
    ...
    if ot_stage.ot_chromium_trial_name:
        json['trial']['origin_trial_feature_name'] = (
            ot_stage.ot_chromium_trial_name
        )
    if ot_stage.ot_is_deprecation_trial:
...
def create_origin_trial(ot_stage: Stage) -> tuple[str | None, str | None]:
    ...
    unique_contacts = [
        email for email in set(unique_contacts) if email.endswith('@google.com')
    ]
...
def activate_origin_trial(origin_trial_id: str) -> None:
    ...
    if settings.DEV_MODE:
        logging.info(
            'Activation request will not be sent to origin trials API in '
            'local environment.'
        )
        return None
    key = settings.OT_API_KEY
...
def extend_origin_trial(trial_id: str, end_milestone: int, intent_url: str):
    ...
    if settings.DEV_MODE:
        logging.info(
            'Extension request will not be sent to origin trials API in '
            'local environment.'
        )
        return
    key = settings.OT_API_KEY
