...
test_app = basehandlers.FlaskApplication(
    ...
    [
        ...
        Route(
            ...
            {'template_path': 'test_template.html', 'name': 'Guest'},
        ),
        Route(
            '/just_an_xml_template',
            basehandlers.ConstHandler,
        ...
        Route(
            '/must_be_signed_in',
            basehandlers.ConstHandler,
            {'template_path': 'test_template.html', 'require_signin': True},
        ),
        Route(
            '/messed_up_template',
            basehandlers.ConstHandler,
...
class BaseHandlerTests(testing_config.CustomTestCase):
    ...
    def test_abort__with_headers__no_msg(self, mock_info):
        ...
        with app.test_request_context('/'):
            try:
                ...
                self.assertEqual('123', e.response.headers.get('X-Test'))
            mock_info.assert_called_once()
    ...
    def test_abort__with_headers__with_msg(self, mock_info):
        ...
        with app.test_request_context('/'):
            try:
                self.handler.abort(
                    401, msg='You messed up', headers={'X-Test': '123'}
                )
                self.fail('Should have raised HTTPException')
            except werkzeug.exceptions.HTTPException as e:
                self.assertEqual(401, e.response.status_code)
    ...
    def test_abort__with_headers__with_500_msg(self, mock_error):
        ...
        with app.test_request_context('/'):
            try:
                self.handler.abort(
                    500, msg='We messed up', headers={'X-Test': '123'}
                )
                self.fail('Should have raised HTTPException')
            except werkzeug.exceptions.HTTPException as e:
    ...
    def test_get_param__simple(self):
        ...
        with test_app.test_request_context('/test', json={'x': 1}):
            ...
            self.assertEqual(
                None, self.handler.get_param('missing', required=False)
            )
            self.assertEqual(
                'usual', self.handler.get_param('missing', default='usual')
            )
    ...
    def test_get_param__missing_required(self, mock_abort):
        ...
        mock_abort.assert_called_once_with(
            400, msg="Missing parameter 'missing'"
        )
    ...
    def test_get_param__validator(self, mock_abort):
        ...
        with test_app.test_request_context('/test', json={'x': 1}):
            ...
            with self.assertRaises(werkzeug.exceptions.BadRequest):
                self.handler.get_param('x', validator=lambda num: num % 2 == 0)
            mock_abort.assert_called_once_with(
...
class APIHandlerTests(testing_config.CustomTestCase):
    ...
    def test_get__dict(self, mock_do_get):
        ...
        self.assertEqual(
            basehandlers.XSSI_PREFIX + '{"key": "value"}',
            response.get_data().decode('utf-8'),
        )
    ...
    def test_require_signed_in_and_xsrf_token__OK_body(
    ...
    @mock.patch('framework.basehandlers.APIHandler.validate_token')
    def test_require_signed_in_and_xsrf_token__missing(
        self, mock_validate_token
    ):
    ...
    def test_update_last_field__no_user(self):
        ...
        updated_invalid_user = self.handler._update_last_visit_field(
...
class FlaskHandlerTests(testing_config.CustomTestCase):
    ...
    def test_get_cache_headers__private(self):
        ...
        self.assertEqual(
            {
                'Cache-Control': (
                    'private, max-age=%s' % settings.DEFAULT_CACHE_TIME
                )
            },
            cache_headers,
        )
    ...
    def test_get_cache_headers__public(self):
        ...
        self.assertEqual(
            {
                'Cache-Control': (
                    'public, max-age=%s' % settings.DEFAULT_CACHE_TIME
                )
            },
            cache_headers,
        )
    ...
    def test_render(self):
        ...
        with test_app.app_context():
            actual = self.handler.render(
                {'name': 'literal'}, 'test_template.html'
            )
    ...
    def test_get__redirect(self):
        ...
        with test_app.test_request_context('/test'):
            actual_response = self.handler.get(redirect_to='some/other/path')
    ...
    def test_post__json(self):
        ...
        self.assertEqual({'objects': [1, 2, 3]}, actual_dict)
        self.assertNotIn('Access-Control-Allow-Origin', actual_headers)
    ...
    def test_split_input(self):
        ...
        with test_app.test_request_context(
            ...
            self.assertEqual(
                ['yellow', 'blue'], self.handler.split_input('colors')
            )
            self.assertEqual(
                ['alice', 'bob'], self.handler.split_input('names', delim=',')
            )
    ...
    def test_split_emails(self):
        ...
        with test_app.test_request_context(
            ...
            self.assertEqual(
                ['user@example.com'], self.handler.split_emails('single')
            )
            self.assertEqual(
    ...
    def test_extract_link__add_http(self):
        ...
        self.assertEqual(
            'http://example.com', self.handler._extract_link('example.com')
        )
        self.assertEqual(
            'http://example.com/1/2/c?x=y&z=2+2',
    ...
    def test_extract_link__bad(self):
        ...
        self.assertIsNone(self.handler._extract_link(None))
        self.assertIsNone(self.handler._extract_link(''))
        self.assertIsNone(self.handler._extract_link('  '))
        self.assertIsNone(self.handler._extract_link('example..com'))
        self.assertIsNone(self.handler._extract_link('TBD'))
        self.assertIsNone(self.handler._extract_link('Coming soon'))
        self.assertIsNone(self.handler._extract_link('http://localhost/'))
        self.assertIsNone(self.handler._extract_link('http://localhost:8080/'))

        self.assertIsNone(self.handler._extract_link('ftp://example.com/ftp'))
        self.assertIsNone(self.handler._extract_link('javascript:alert(1)'))
        self.assertIsNone(
            self.handler._extract_link('javascript:window.alert(1)')
        )
        self.assertIsNone(self.handler._extract_link('about:flags'))

        # We might add support for these sometime, but not now.
        self.assertIsNone(self.handler._extract_link('b/1234'))
        self.assertIsNone(self.handler._extract_link('go/1234'))
    ...
    def test_parse_link(self):
        ...
        with test_app.test_request_context(
            ...
            self.assertEqual(
                'http://example.com', self.handler.parse_link('plain')
            )
            self.assertEqual(
                'http://example.com', self.handler.parse_link('noscheme')
            )
            self.assertEqual(
                'http://example.com', self.handler.parse_link('withspace')
            )
            self.assertEqual(
                'http://example.com', self.handler.parse_link('extrajunk')
            )
    ...
    def test_parse_links(self):
        ...
        with test_app.test_request_context(
            ...
            self.assertEqual(
                ['https://example.com'], self.handler.parse_links('single')
            )
            self.assertEqual(
...
class GetSPATemplateDataTests(testing_config.CustomTestCase):
    ...
    def test_get_spa_template_data__signin_missing(self):
        ...
        with test_app.test_request_context('/must_be_signed_in'):
            ...
            actual_redirect, actual_headers = (
    ...
    def test_get_spa_template_data__signin_missing_after_redirect(self):
        ...
        testing_config.sign_out()
        with test_app.test_request_context(
            '/must_be_signed_in?loginStatus=False'
        ):
            defaults = {'require_signin': True}
    ...
    def test_get_spa_template_data__create_perm_missing(self):
        ...
        self.assertEqual(302, actual_redirect.status_code)
        self.assertEqual(
            '/features?error=403', actual_redirect.headers['location']
        )
    ...
    def test_get_spa_template_data__edit_perm_no_permission(self):
        ...
        self.assertEqual(302, actual_redirect.status_code)
        self.assertEqual(
            f'/feature/{self.fe_1.key.integer_id()}?error=403',
            actual_redirect.headers['location'],
        )
