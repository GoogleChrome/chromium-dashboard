...
class BaseHandler(flask.views.MethodView):
    ...
    def get_int_param(
        ...
        if val and type(val) != int:  # noqa: E721
    ...
    def get_int_arg(self, name, default=None) -> int | None:
        ...
        if num < 0:
            self.abort(
                400, msg='Request parameter %r out of range: %r' % (name, val)
            )
        return num
...
class EntitiesAPIHandler(APIHandler):
    ...
    def abort_invalid_data_type(
        self, field: str, field_type: str, value: Any
    ) -> None:
    ...
    def update_stage(
        ...
        mutating_ot_milestones = any(
            isinstance(v, dict)
            and (
                v['form_field_name'] == 'ot_milestone_desktop_start'
                or v['form_field_name'] == 'ot_milestone_desktop_end'
            )
        ...
        if mutating_ot_milestones and ot_creation_in_progress:
            self.abort(
                400, 'Cannot edit OT milestones while creation is in progress.'
            )
...
class FlaskHandler(BaseHandler):
    ...
    def get_common_data(self, path=None):
        ...
        if user:
            ...
            approvers = approval_defs.get_approvers(gate_type)  # noqa: F841
    ...
    def get(self, *args, **kwargs):
        ...
        if self.JSONIFY and type(handler_data) in (dict, list):
        ...
        elif type(handler_data) == dict:  # noqa: E721
            ...
            template_text = self.render(
                handler_data, os.path.join(template_path)
            )
            headers = self.get_headers()
    ...
    def require_task_header(self):
        ...
        self.abort(
            403,
            msg=(
                'Lacking X-AppEngine-QueueName or '
    ...
    def get_template_data(self, **defaults):
        ...
        if 'template_path' in defaults:
            ...
            if not template_path.endswith(('.html', '.xml')):
                self.abort(
                    500,
                    msg=f'${template_path =} does not end with .html or .xml',
                )
            return defaults
...
def get_spa_template_data(handler_obj, defaults):
    ...
    requires_signin = (
        defaults.get('require_signin')
        or defaults.get('require_create_feature')
        or defaults.get('require_edit_feature')
        or defaults.get('require_admin_site')
        or defaults.get('is_enterprise_page')
    )
...
def FlaskApplication(import_name, routes, pattern_base='', debug=False):
    ...
    app = flask.Flask(
        import_name, template_folder=settings.get_flask_template_path()
    )
