...
the entry is saved with a property_name === 'ERROR'. The histogram buckets are
updated by a nightly cron job (/cron/histograms), but sometimes changes to the
enums.xml can cause data not make it into a proper bucket. This script finds the
invalid. entities in the datastore and corrects their bucket_id.
