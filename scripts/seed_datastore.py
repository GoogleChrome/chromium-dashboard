...
into the development datastore this script can also set a user as admin, to
facilitate testing those parts of the site.
"""  # noqa: D205
...
def add_features(server: str, after: datetime, detailsAfter: datetime):
    """Fetches features and inserts them into the dev datastore data is fetched
    ...
    logging.info(
        'Adding %d recent features of %d total to the datastore.',
        len(recent_features),
        len(parsed_features),
    )
    ...
    for f in recent_features:
        ...
        if fe.updated > detailsAfter:
            ...
            if details['accurate_as_of'] is not None:
                fe.accurate_as_of = datetime.fromisoformat(
                    details['accurate_as_of']
                )
            ...
            fe.initial_public_proposal_url = details[
            ...
            for stage in details['stages']:
                ...
                s.ot_feedback_submission_url = stage[
                    'ot_feedback_submission_url'
                ]
                s.ot_has_third_party_support = stage[
                ...
                s.ot_use_counter_bucket_number = stage[
                    'ot_use_counter_bucket_number'
                ]
                s.ot_webfeature_use_counter = stage['ot_webfeature_use_counter']
                s.experiment_extension_reason = stage[
...
parser.add_argument(
    '--admin', help='email address of the user to make a site admin'
)
parser.add_argument(
    '--after',
...
parser.add_argument(
    '--details-after',
    type=datetime.fromisoformat,
    help='fetch more details for each feature that was updated since '
    'DETAILS-AFTER',
    default='9999-01-01',
)
