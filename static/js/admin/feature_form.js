...
// A object graph mapping form fields to implementation status and whehter or
...
function intentStageChanged(stage) {
  function set_required(id, required) {
    if (required) {
  ...
  function hide(id) {
    ...
    document.querySelector('#id_' + id).parentNode.parentNode
	.style = "display: none;";
  }

  let stage_index = INTENT_IDENTIFIER_NAMES[Number(stage.value)];

  for (let id in FORM_FIELD_GRAPH) {
    if (FORM_FIELD_GRAPH[id][stage_index][FORM_FIELD_SHOW_POS]) {
      show(id);
    } else {
      hide(id);
    }

    if (FORM_FIELD_GRAPH[id][stage_index][FORM_FIELD_REQUIRED_POS]) {
      set_required(id, true);
    } else {
      set_required(id, false);
    }
  }

  let intent_stage_name = document.querySelector('#id_intent_stage_name')

  if (intent_stage_name) {
    intent_stage_name.textContent =
