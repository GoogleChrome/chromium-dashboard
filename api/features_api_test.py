...
class FeaturesAPITest(testing_config.CustomTestCase):
    def setUp(self):
        ...
        self.enterprise_stage = Stage(
            id=60, feature_id=self.feature_1_id, stage_type=1061
        )
        ...
        self.feature_2 = FeatureEntry(
            ...
            summary='sum K',
            feature_type=1,
            owner_emails=['other_owner@example.com'],
            category=1,
            intent_stage=core_enums.INTENT_IMPLEMENT,
        )
        self.feature_2.put()
        self.feature_2_id = self.feature_2.key.integer_id()
        self.ship_stage_2 = Stage(
        ...
        self.ship_stage_3 = Stage(
            feature_id=self.feature_3_id,
            stage_type=360,
            milestones=MilestoneSet(desktop_first=2),
        )
        ...
        self.feature_confidential = FeatureEntry(
            name='feature confidential',
            summary='sum A',
            feature_type=2,
            owner_emails=['confidential_owner@example.com'],
            category=1,
    ...
    def test_get__all_listed_feature_names(self):
        ...
        self.assertEqual(2, len(actual['features']))
        self.assertEqual(2, actual['total_count'])
        self.assertEqual(6, len(actual['features'][0]))
        self.assertEqual('feature two', actual['features'][0]['name'])
        self.assertEqual(6, len(actual['features'][1]))
        self.assertEqual('feature one', actual['features'][1]['name'])
    ...
    def test_get__in_milestone_listed(self):
        ...
        self.assertEqual(1, actual['total_count'])
        self.assertEqual(
            1, len(actual['features_by_type']['Enabled by default'])
        )
        ...
        self.assertEqual(
            0, len(actual['features_by_type']['Enabled by default'])
        )
    ...
    def test_get__in_milestone_unlisted_can_edit(self):
        ...
        self.assertEqual(2, actual['total_count'])
        self.assertEqual(
            2, len(actual['features_by_type']['Enabled by default'])
        )
    ...
    def test_patch__valid(self):
        ...
        expected_changes = [
            ...
            (
                'doc_links',
                ['https://example.com/docs1', 'https://example.com/docs2'],
            ),
            ('category', 1),
        ...
        with test_app.test_request_context(
            request_path, json=valid_request_body
        ):
    ...
    def test_patch__invalid_fields(self):
        ...
        request_path = f'{self.request_path}/update'
        with test_app.test_request_context(
            request_path, json=invalid_request_body
        ):
            self.handler.do_patch(feature_id=self.feature_1_id)
    ...
    def test_patch__accurate_as_of(self):
        ...
        accuracy_activity = next(
            (
                ...
                for a in activities
                if any(
                    amend.field_name == 'accurate_as_of'
                    for amend in a.amendments
                )
            ),
            None,
        )
    ...
    def test_patch__enterprise_first_notice_enterprise_feature(self, mock_call):
        ...
        next_stable_date = _datetime_to_str(
            datetime.now().replace(year=datetime.now().year + 1, day=1)
        )
        mock_call.return_value = {
            100: {'version': 100, 'stable_date': stable_date},
    ...
    def test_patch__enterprise_first_notice_becomes_not_breaking_feature(
        ...
        first_enterprise_notification_milestone deleted.
        """
        stable_date = _datetime_to_str(
            datetime.now().replace(year=datetime.now().year + 1, day=1)
        )
    ...
    def test_patch__first_notice_becomes_not_breaking_feature_already_published(
        ...
        """PATCH request successful with
        first_enterprise_notification_milestone not deleted.
        """
        stable_date = _datetime_to_str(
            datetime.now().replace(year=datetime.now().year - 1, day=1)
        )
    ...
    def test_post__valid(self):
        ...
        self.assertTrue(type(response['feature_id']) == int)  # noqa: E721
    ...
    def test_post__valid_stage_and_gate_creation(self):
        ...
        stages = Stage.query(
            Stage.feature_id == new_feature.key.integer_id()
        ).fetch()
        gates = Gate.query(
    ...
    def test_post__first_enterprise_notification_milestone_missing_enterprise(
        ...
        self.assertEqual(
            new_feature.first_enterprise_notification_milestone, 420
        )
    ...
    def test_post__first_enterprise_notification_milestone_set(self, mock_call):
        ...
        self.assertEqual(
            new_feature.first_enterprise_notification_milestone, 123
        )
