...
URL_RE = re.compile(
    r'\b%s%s%s\b' % (SCHEME_PATTERN, DOMAIN_PATTERN, PATH_PARAMS_ANCHOR_PATTERN)
)
...
class FeaturesAPI(basehandlers.EntitiesAPIHandler):
    ...
    def do_search(self):
        ...
        if milestone:
            ...
            total_count = sum(
                len(features_by_type[t]) for t in features_by_type
            )
            return {
        ...
        if release_notes_milestone:
            features_in_release_notes = (
                feature_helpers.get_features_in_release_notes(
    ...
    def _post_update_special_fields(
        ...
        if feature.feature_type == FEATURE_TYPE_ENTERPRISE_ID:  # noqa: F405
            ...
            feature.tag_review_status = REVIEW_NA  # noqa: F405
        else:
    ...
    def _write_stages_and_gates_for_feature(
        ...
        stages_gates = STAGES_AND_GATES_BY_FEATURE_TYPE[feature_type]  # noqa: F405

        for stage_type, gate_types in stages_gates:
            # Don't create a trial extension stage pre-emptively.
            if stage_type == STAGE_TYPES_EXTEND_ORIGIN_TRIAL[feature_type]:  # noqa: F405
                continue
    ...
    def _patch_update_stages(
        ...
        # for basehandlers.update_stage(). This logic is mostly duplicated otherwise.  # noqa: E501
        ...
        for change_info in stage_changes_list:
            ...
            for field, field_type in api_specs.MILESTONESET_FIELD_DATA_TYPES:
                ...
                if (
                    ...
                    or form_field_name == 'shipped_android_milestone'
                    or form_field_name == 'shipped_ios_milestone'
                    or form_field_name == 'shipped_webview_milestone'
                ):
                    ship_milestones_were_updated = True

                self.update_field_value(
                    milestones, field, field_type, new_value
                )
            ...
            if (
                'rollout_platforms' in change_info
                or 'rollout_details' in change_info
            ):
                ship_milestones_were_updated = True
    ...
    def _maybe_reset_releasenotes_flags(
        self, feature: FeatureEntry, changed_fields: CHANGED_FIELDS_LIST_TYPE
    ) -> None:
    ...
    def _patch_update_special_fields(
        ...
        if 'accurate_as_of' in feature_changes:
            changed_fields.append(
                ('accurate_as_of', feature.accurate_as_of, now)
            )
        ...
        has_updated |= self._shared_update_special_fields(
            feature, feature_changes
        )
        # Set enterprise first notification milestones.
        if is_update_first_notification_milestone(feature, feature_changes):  # noqa: F405
            feature.first_enterprise_notification_milestone = int(
                feature_changes['first_enterprise_notification_milestone']
            )  # noqa: E501
            has_updated = True
        ...
        if updated_shipping_stages:
            ...
            shipping_stage_dict = {
                es.key.integer_id(): es for es in existing_shipping_stages
            }
            shipping_stage_dict.update(
                {uss.key.integer_id(): uss for uss in updated_shipping_stages}
            )
            ...
            if earliest:
                ...
                if year != feature.shipping_year:
                    changed_fields.append(
                        ('shipping_year', feature.shipping_year, year)
                    )  # noqa: E501
        ...
        if stage_ids:
            stage_type_to_stage_ids = stage_helpers.get_feature_stage_ids(
                feature_id
            )
            ...
            active_stage_type = stage_id_to_stage_type.get(
                feature.active_stage_id, 0
            )
            highest_edited_stage_id = max(
                stage_ids, key=lambda s: stage_id_to_stage_type[s]
            )
            highest_edited_stage_type = stage_id_to_stage_type[
                highest_edited_stage_id
            ]  # noqa: E501
    ...
    def do_patch(self, **kwargs) -> flask.Response | dict[str, str]:
        ...
        # Reset outstanding notifications if the user updated any ship/rollout milestones.  # noqa: E501
