...
class ExternalReviewsAPITest(testing_config.CustomTestCase):
        ...
        if milestones is not None:
            ...
            for field_name, _type in MILESTONESET_FIELD_DATA_TYPES:
                ...
                if value is not None:
                    stage_info[field_name] = dict(
                        form_field_name=field_name, value=value
                    )
    ...
    def test_e2e_features_that_need_review_are_included(
        ...
        def github_result(link):
            ...
            if (
                link.url
                == 'https://github.com/mozilla/standards-positions/issues/1'
            ):
                result.update(
                    number=1,
                ...
                link.url
                == 'https://github.com/mozilla/standards-positions/issues/2'
            ):
                result.update(number=2)
            elif (
                link.url
                == 'https://github.com/mozilla/standards-positions/issues/3'
            ):
                result.update(number=3)
            elif (
                link.url
                == 'https://github.com/mozilla/standards-positions/issues/4'
            ):
                ...
                link.url
                == 'https://github.com/WebKit/standards-positions/issues/6'
            ):
                ...
                result.update(
                    number=4, state='closed', closed_at='2024-04-20T07:15:34'
                )
            else:
