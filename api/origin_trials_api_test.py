...
"""
    ...
    def test_get__valid(self, mock_get_trials_list):
        ...
        self.assertEqual(
            GetOriginTrialsResponse.from_dict(
                {
                    'origin_trials': [
                        {'id': '123', 'display_name': 'Example Trial'}
                    ]
    ...
    def test_do_post__stage_not_found(self):
        ...
        kwargs = {'feature_id': str(self.feature_1_id), 'stage_id': '999'}
        with test_app.test_request_context(self.request_path):
            with self.assertRaises(werkzeug.exceptions.NotFound):
                self.handler.do_post(**kwargs)
    ...
    def test_post__valid(self, mock_get_chromium_files, mock_validate_func):
        ...
        self.assertEqual(
            response, {'message': 'Origin trial creation request submitted.'}
        )
        ...
        for field, field_info in body.items():
            ...
            if field == 'ot_emails':
                ...
                field == 'ot_approval_buganizer_component'
                or field == 'ot_approval_buganizer_custom_field_id'
            ):
                value = int(value)
            # Check milestone fields
            elif field == 'desktop_first' or field == 'desktop_last':
                self.assertEqual(
                    getattr(self.ot_stage_1.milestones, field), value
                )
                continue
