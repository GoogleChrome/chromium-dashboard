...
class VotesAPI(basehandlers.APIHandler):
    ...
    def do_post(self, **kwargs) -> dict[str, str]:
        ...
        new_gate_state = approval_defs.set_vote(
            feature_id, None, new_state, user.email(), gate_id
        )
    ...
    def require_permissions(self, user, feature, gate, new_state):
        ...
        is_requesting_review = new_state in (
...
class PendingGatesAPI(basehandlers.APIHandler):
    ...
    def do_get(self, **kwargs):
        ...
        if type(future_projections) == list:  # noqa: E721
        ...
        dicts = [converters.gate_value_to_json_dict(g) for g in gates]
        for g in dicts:
            g['possible_assignee_emails'] = prefetched_approvers.get(
                g['gate_type'], []
            )  # noqa: E501
...
class XfnGatesAPI(basehandlers.APIHandler):
    ...
    def create_xfn_gates(self, feature_id, stage_id) -> int:
        """Create all new incubation gates on a PSA stage"""  # noqa: D415
        logging.info('Creating xfn gates')
        existing_gates = Gate.query(
            Gate.feature_id == feature_id, Gate.stage_id == stage_id
        ).fetch()
