...
"""API handlers for retrieving the lists of owners and.

Subscribers across all Blink components.
"""
...
class ComponentsUsersAPI(basehandlers.APIHandler):
    ...
    def do_get(self, **kwargs) -> dict:
        ...
        With 1) subscribers for each component and 2) each component.
        """
        ...
        users = [
            ComponentsUser(id=fo.key.integer_id(), email=fo.email, name=fo.name)
        ...
        for ps in possible_subscribers:
            for subed_component_key in ps.blink_components:
                component_to_subscribers[subed_component_key].append(
                    ps.key.integer_id()
                )
            for owned_component_key in ps.primary_blink_components:
                component_to_owners[owned_component_key].append(
                    ps.key.integer_id()
                )
