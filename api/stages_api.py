...
"""API endpoints for creating.

Retrieving, updating, and deleting feature stages.
"""
...
class StagesAPI(basehandlers.EntitiesAPIHandler):
    ...
    def _create_stage(
        self, feature_id: int, feature_type: int, stage_type: int
    ):
    ...
    def _validate_edit_permissions(self, feature_id: int, request_body: dict):
        ...
        if (
            not user
            or not is_ot_request
            or not (
                user.email().endswith('@chromium.org')
                or user.email().endswith('@google.com')
            )
        ):
