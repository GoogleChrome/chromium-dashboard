...
from internals.data_types import (
...
def _prep_stage_info(
    ...
    for extension in extension_stages:
        ...
        if ot_id and ot_id in ot_stage_indexes:
            (
                stage_info['all_stages'][ot_stage_indexes[ot_id]][
                    'extensions'
                ].append(extension)
            )
...
def stage_to_json_dict(
    stage: Stage, feature_type: int | None = None
) -> StageDict:
...
def _format_new_crbug_url(
    blink_components: Optional[list[str]],
    bug_url: Optional[str],
    impl_status_chrome: int,
    owner_emails: list[str] = list(),
) -> str:
...
def feature_entry_to_json_verbose(
    ...
    d: VerboseFeatureDict = {
        ...
        'tag_review_status': REVIEW_STATUS_CHOICES.get(
            ...
            REVIEW_STATUS_CHOICES[REVIEW_PENDING],
        ),
        ...
        'browsers': {
            'chrome': {
                ...
                'android': _get_milestone_attr(
                    stage_info['ship'], 'android_first'
                ),
                'webview': _get_milestone_attr(
                    stage_info['ship'], 'webview_first'
                ),
...
def feature_entry_to_json_basic(
    fe: FeatureEntry, stages: list[Stage] | None = None
) -> dict[str, Any]:
    ...
    if stages and d['is_released']:
        for s in stages:
            if (
                s.stage_type == STAGE_TYPES_SHIPPING[fe.feature_type]
                and s.milestones is not None
            ):
                milestone = (
                    s.milestones.desktop_first
                    or s.milestones.android_first
                    or s.milestones.ios_first
                    or s.milestones.webview_first
                )
...
def gate_value_to_json_dict(gate: Gate) -> dict[str, Any]:
    ...
    if requested_on:
        ...
        if gate.resolved_on:
            slo_resolve_took = max(
                0,
                slo.weekdays_between(gate.requested_on, gate.resolved_on)
                - (gate.needs_work_elapsed or 0),
            )
        elif (
            gate.state != Vote.NEEDS_WORK
            and gate.state not in Gate.FINAL_STATES
        ):
            slo_resolve_remaining = slo.remaining_days(
                gate.requested_on, slo_resolve
            ) + (gate.needs_work_elapsed or 0)
