...
"""Tests for the attachments_api module verifying upload permissions and
serving.
"""
...
class AttachmentServingTest(testing_config.CustomTestCase):
    ...
    def test_maybe_redirect__alt_base(self):
        ...
        with test_app.test_request_context(
            self.request_path, base_url=alt_base
        ):
            actual = self.handler.maybe_redirect(self.attachment, False)
        ...
        with test_app.test_request_context(self.request_path):
            with self.assertRaises(werkzeug.exceptions.NotFound):
                self.handler.get_template_data(
                    feature_id=self.feature_id,
                    attachment_id=self.attachment_id + 1,
                )
            with self.assertRaises(werkzeug.exceptions.NotFound):
                self.handler.get_template_data(
                    feature_id=self.feature_id + 1,
...
class RoundTripTest(testing_config.CustomTestCase):
    def setUp(self):
        ...
        self.api_request_path = (
            f'/api/v0/features/{self.feature_id}/attachments'
        )
